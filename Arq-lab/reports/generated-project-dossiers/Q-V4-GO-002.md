# Generated Project Dossier - Q-V4-GO-002

## 1. Scenario Identity

- scenarioId: `Q-V4-GO-002`
- scenarioName: Quantum weak crypto patterns in a realistic Go provisioning service.
- milestone: `M4`
- targetModule: `Quantum`
- language / stack: `Go`
- repoType: `snapshot`
- repo local path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\M4\device-provisioning-go`
- repo remote URL in Gitea: `http://localhost:3001/arq/device-provisioning-go-20260330t134454z`
- default branch: `main`
...
This scenario models `Device bootstrap credential service.` as a `Go` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Quantum weak crypto patterns in a realistic Go provisioning service.`.

## 3. Architecture Summary

- Major components: `cmd, docs, internal, ops, sql, validation`
- Runtime role: `Device bootstrap credential service.`
...
- Path: `internal/security/legacy_hash.go`
  ...
  Explainability expectation: resolvedValue~MD5, queryFamily~go; resolvedValue~SHA1
- Path: `internal/security/legacy_hash.go`
...
```text
0001: # device-provisioning-go
0002: 
0003: Device bootstrap credential service.
0004: 
...
### `internal/security/secure_hmac.go`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business module contributing to Secure Hmac. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: package security
0002: import "crypto/sha256"
0003: func SecureDigest(input []byte) [32]byte { return sha256.Sum256(input) }
```
...
- path: `legacy_hash.go`
  ...
  expected explainability behavior: resolvedValue~MD5, queryFamily~go; resolvedValue~SHA1
- path: `legacy_hash.go`
  module: `Quantum`
...
- path: `docs/compatibility-notes.md`
  why finding must not appear: scenario declares this path clean despite adversarial content or naming.
  negative type: `near-real negative`
...
- `legacy_hash.go`: resolvedValue~`MD5`, queryFamily~`go`
