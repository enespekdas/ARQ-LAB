# Generated Project Dossier - G-V2-HIST-003

## 1. Scenario Identity

- scenarioId: `G-V2-HIST-003`
- scenarioName: Guardian PEM rotation, canonicalization and dedup continuity.
- milestone: `M2`
...
- repo remote URL in Gitea: `http://localhost:3001/arq/pem-rotation-lineage-20260330t172732z`
- default branch: `main`
...
This scenario models `PEM rotation and dedup lineage validation.` as a `Python + ops config` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Guardian PEM rotation, canonicalization and dedup continuity.`.

## 3. Architecture Summary

- Major components: `app, docs, ops, scripts, sql, tests, validation`
- Runtime role: `PEM rotation and dedup lineage validation.`
- Config flow: `No dedicated live-config files; runtime behavior is code-driven.`
- Secret flow: `ops/keys/service-a.pem, ops/keys/service-b.pem`
...
- Path: `ops/keys/service-a.pem`
  Why it should be detected: scenario declares `rotated-key-a` as a live positive surface.
...
- Path: `ops/keys/service-b.pem`
  Why it should be detected: scenario declares `rotated-key-b` as a live positive surface.
  Target module: `Guardian`
  Finding family / rule family expectation: `guardian.private-key`
  Head/history behavior: `HEAD_AND_HISTORY`
  Explainability expectation: No strict scenario-specific explainability contract beyond normal detail capture.
...
- `main` tip: `324fe025e8b71618622f87e680df9b534c3ff062`

Commit order:

- `e7b6842afe06c4e4c0ca073b0b2cf86e0e073f18` `c001 bootstrap`: initial clean or baseline assembly.
- `0d5bb35e1109899023a13902367c72a500c96920` `c002 add PEM A`: introduces an intended signal.
- `f484dad1c0baec8af6996ed36f868d50fb57ae86` `c003 formatting change`: scenario state change.
- `64d90b241a5d75e2a0429155cf0b6dfd68b815df` `c004 rotate to PEM B`: replaces one signal with another.
- `abe526c9e77436733ea179454d91a5d9569a1470` `c005 docs example PEM`: adds documentation-only or noise-only collateral.
- `324fe025e8b71618622f87e680df9b534c3ff062` `c006 safe cleanup`: removes or neutralizes a prior signal.

Expected final head/history state:

- `rotated-key-a` on `ops/keys/service-a.pem` should behave as `HISTORY_ONLY`.
- `rotated-key-b` on `ops/keys/service-b.pem` should behave as `HEAD_AND_HISTORY`.
...
```text
0001: # pem-rotation-lineage
0002: 
0003: PEM rotation and dedup lineage validation.
0004: 
...
### `docs/examples/pem-reference.md`

- Why this file matters: `docs` file with expectation `must_not_find`.
- Detailed summary: Documentation or explanatory material for Pem Reference. It is non-live or protected in the assembled repository.
...
- Detailed summary: Generated or derived project artifact related to Service B. It is non-live or protected in the assembled repository.
...
- path: `ops/keys/service-b.pem`
  module: `Guardian`
  expected rule/finding family: `guardian.private-key`
  expected branch/ref behavior: `HEAD_AND_HISTORY`
  expected explainability behavior: No strict scenario-specific explainability contract beyond normal detail capture.

### must_not_find

- path: `docs/examples/pem-reference.md`
