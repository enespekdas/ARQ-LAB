# Generated Project Dossier - Q-V5-PY-004

## 1. Scenario Identity

- scenarioId: `Q-V5-PY-004`
- scenarioName: Quantum should surface live verify=False through kwargs passthrough and wrapper indirection in Python.
- milestone: `M5`
- targetModule: `Quantum`
- language / stack: `Python / client wrapper`
- repoType: `snapshot`
- repo local path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-kwargs-python`
- repo remote provider: `github`
- repo remote URL: `https://github.com/ARQ-Sec/tls-kwargs-python`
- default branch: `main`
- scan modes intended for this scenario: `HEAD_SNAPSHOT`
- branch scopes intended for this scenario: `SINGLE_BRANCH`
- project-local dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-kwargs-python\validation\generated-project-dossier.md`
- required alias dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\Q-V5-PY-004\validation\generated-project-dossier.md`
- central dossier report path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\reports\generated-project-dossiers\Q-V5-PY-004.md`

## 2. Business Context and Why This Repo Is Realistic

This scenario models `Python repo where verify=False flows through kwargs passthrough and a session wrapper.` as a `Python / client wrapper` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Quantum should surface live verify=False through kwargs passthrough and wrapper indirection in Python.`.

## 3. Architecture Summary

- Major components: `app, docs, examples, ops, scripts, sql, tests, validation`
- Runtime role: `Python repo where verify=False flows through kwargs passthrough and a session wrapper.`
...
- Crypto/TLS flow if relevant: `session_wrapper.py`
...
| app/clients/safe_session_wrapper.py | live-code | 4 | Runtime business module contributing to Safe Session Wrapper. | no | yes | no | yes | yes | no |
| app/clients/session_wrapper.py | live-code | 10 | Runtime business module contributing to Session Wrapper. | yes | no | no | yes | yes | no |
...
| examples/session_example.py | docs | 2 | Documentation or explanatory material for Session Example. | no | no | yes | no | no | no |
...
| validation/generated-project-dossier.md | generated | 754 | Machine-readable validation contract or generated audit artifact for this scenario. | no | no | yes | no | no | no |
...
- Path: `app/clients/session_wrapper.py`
  Why it should be detected: scenario declares `kwargs-verify-false` as a live positive surface.
...
- `app/clients/safe_session_wrapper.py`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
...
| examples/session_example.py | docs | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
...
- Command: `C:\Python313\python.EXE -m compileall app tests scripts`
  ...
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-kwargs-python\validation\runnability-logs\build-01.log`
...
- Command: `C:\Python313\python.EXE -m pytest -q`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-kwargs-python\validation\runnability-logs\test-01.log`
...
- Command: `C:\Python313\python.EXE scripts/smoke.py`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-kwargs-python\validation\runnability-logs\smoke-01.log`
...
```text
0002: 
0003: Python repo where verify=False flows through kwargs passthrough and a session wrapper.
0004: 
```

### `app/clients/safe_session_wrapper.py`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business module contributing to Safe Session Wrapper. It is executable/live in the assembled repository.
...
0004:     return requests.get(url, verify=True, timeout=5).text
```

### `app/clients/session_wrapper.py`

- Why this file matters: `live-code` file with expectation `must_find`.
- Detailed summary: Runtime business module contributing to Session Wrapper. It is executable/live in the assembled repository.
...
0002: 
0003: 
0004: def _transport(url: str, **kwargs) -> str:
0005:     return requests.get(url, timeout=5, **kwargs).text
0006: 
0007: 
0008: def fetch_payload(url: str) -> str:
0009:     options = {'verify': False}
0010:     return _transport(url, **options)
```
...
- Detailed summary: Documentation or explanatory material for Session Example. It is non-live or protected in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`False`; protected=`True`.
- Representative excerpt:

```text
0001: def example() -> dict[str, bool]:
0002:     return {'verify': False}
```
...
| category | LOC |
| --- | ---: |
| live business code | 99 |
| live config | 0 |
| tests | 32 |
| docs | 13 |
| scripts | 1 |
| fixtures | 0 |
| vendor/generated | 141 |
...
- path: `examples/session_example.py`
  why finding must not appear: scenario declares this path clean despite adversarial content or naming.
...
- `session_wrapper.py`: resolvedValue~`verify=False`, queryFamily~`ssl`
...
- What this scenario is proving: `Quantum should surface live verify=False through kwargs passthrough and wrapper indirection in Python.`
