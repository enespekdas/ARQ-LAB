# Generated Project Dossier - Q-V5-PY-002

## 1. Scenario Identity

- scenarioId: `Q-V5-PY-002`
- scenarioName: Quantum TLS trust bypass patterns in Python clients.
- milestone: `M5`
...
This scenario models `Data sync worker pulling partner APIs and webhooks.` as a `Python / worker service` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Quantum TLS trust bypass patterns in Python clients.`.

## 3. Architecture Summary

- Major components: `app, docs, ops, scripts, sql, tests, validation`
- Runtime role: `Data sync worker pulling partner APIs and webhooks.`
...
- Crypto/TLS flow if relevant: `partner_pull_client.py, unverified_ssl_context.py`
...
|-- app
|   |-- clients
|   |   |-- partner_pull_client.py
|   |   `-- secure_partner_pull_client.py
|   |-- models
|   |   |-- audit.py
|   |   |-- delivery.py
|   |   |-- dispatch.py
...
| app/clients/partner_pull_client.py | live-code | 4 | Outbound integration client for Partner Pull Client behavior. | yes | no | no | yes | yes | no |
| app/clients/secure_partner_pull_client.py | live-code | 2 | Outbound integration client for Secure Partner Pull Client behavior. | no | yes | no | yes | yes | no |
...
- Path: `app/clients/partner_pull_client.py`
  Why it should be detected: scenario declares `verify-false` as a live positive surface.
...
- Path: `app/security/unverified_ssl_context.py`
  Why it should be detected: scenario declares `unverified-context` as a live positive surface.
  Target module: `Quantum`
  Finding family / rule family expectation: `ssl._create_unverified_context`
  Head/history behavior: `head-only`
  Explainability expectation: resolvedValue~ssl._create_unverified_context
...
```text
0001: # data-sync-python
0002: 
0003: Data sync worker pulling partner APIs and webhooks.
0004: 
...
### `app/clients/partner_pull_client.py`

- Why this file matters: `live-code` file with expectation `must_find`.
- Detailed summary: Outbound integration client for Partner Pull Client behavior. It is executable/live in the assembled repository.
...
### `app/clients/secure_partner_pull_client.py`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Outbound integration client for Secure Partner Pull Client behavior. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: def fetch_payload(url: str) -> str:
0002:     return url
```
...
### `app/security/unverified_ssl_context.py`
...
```text
0003: def build_context():
0004:     return ssl._create_unverified_context()
```
...
- path: `unverified_ssl_context.py`
  module: `Quantum`
  expected rule/finding family: `ssl._create_unverified_context`
  expected branch/ref behavior: `head-only`
  expected explainability behavior: resolvedValue~ssl._create_unverified_context

### must_not_find

- path: `secure_partner_pull_client.py`
...
- `partner_pull_client.py`: resolvedValue~`verify=False`, queryFamily~`ssl`
- `unverified_ssl_context.py`: resolvedValue~`ssl._create_unverified_context`
