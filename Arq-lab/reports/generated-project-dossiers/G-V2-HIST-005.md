# Generated Project Dossier - G-V2-HIST-005

## 1. Scenario Identity

- scenarioId: `G-V2-HIST-005`
- scenarioName: Guardian history handling when the same secret is reformatted, moved, and later removed.
- milestone: `M2`
- targetModule: `Guardian`
- language / stack: `Python + ops runtime`
- repoType: `history`
- repo local path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\canonical-secret-lineage`
- repo remote provider: `github`
- repo remote URL: `https://github.com/ARQ-Sec/canonical-secret-lineage`
- default branch: `main`
- scan modes intended for this scenario: `HEAD_SNAPSHOT, REF_HISTORY`
- branch scopes intended for this scenario: `SINGLE_BRANCH`
- project-local dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\canonical-secret-lineage\validation\generated-project-dossier.md`
- required alias dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\G-V2-HIST-005\validation\generated-project-dossier.md`
- central dossier report path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\reports\generated-project-dossiers\G-V2-HIST-005.md`

## 2. Business Context and Why This Repo Is Realistic

This scenario models `Runtime token lineage with formatting changes and dedup-like history on main.` as a `Python + ops runtime` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Guardian history handling when the same secret is reformatted, moved, and later removed.`.

## 3. Architecture Summary

- Major components: `app, docs, ops, scripts, sql, tests, validation`
- Runtime role: `Runtime token lineage with formatting changes and dedup-like history on main.`
- Config flow: `ops/archive/partner_runtime.env`
- Secret flow: `partner_runtime.env`
...
| app/security/rotation_state.py | live-code | 2 | Runtime business module contributing to Rotation State. | no | yes | no | yes | yes | no |
...
| docs/examples/runtime-reference.md | docs | 1 | Documentation or explanatory material for Runtime Reference. | no | no | yes | no | no | no |
| ops/archive/partner_runtime.env | live-config | 1 | Runtime configuration carrying environment or deployment settings for Partner Runtime. | yes | no | no | yes | yes | no |
...
- Path: `ops/archive/partner_runtime.env`
  Why it should be detected: scenario declares `history-runtime-token` as a live positive surface.
...
- `app/security/rotation_state.py`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
...
| docs/examples/runtime-reference.md | docs | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
...
- `main` tip: `626f417fec2393a2e923648b0cfca84cee81b2b0`

Commit order:

- `82058f59bc82066e80f93f9c15e51f116f587651` `c001 bootstrap`: initial clean or baseline assembly.
- `2558dfa25dea50157298133aa5358924404a70aa` `c002 add runtime token`: introduces an intended signal.
- `013ec5408960f0eafc444419bc04e3b15478e031` `c003 reformat runtime token`: scenario state change.
- `753b993d9276b082473b67dc264817566ad69603` `c004 move runtime token to archive`: scenario state change.
- `89b6b2e2a27e94ad894d12987e21570126ba400d` `c005 add runtime docs example`: introduces an intended signal.
- `626f417fec2393a2e923648b0cfca84cee81b2b0` `c006 remove runtime token and keep safe placeholder`: removes or neutralizes a prior signal.

Expected final head/history state:

- `history-runtime-token` on `partner_runtime.env` should behave as `HISTORY_ONLY`.
...
- Command: `C:\Python313\python.EXE -m compileall app tests scripts`
  ...
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\canonical-secret-lineage\validation\runnability-logs\build-01.log`
...
- Command: `C:\Python313\python.EXE -m pytest -q`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\canonical-secret-lineage\validation\runnability-logs\test-01.log`
...
- Command: `C:\Python313\python.EXE scripts/smoke.py`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\canonical-secret-lineage\validation\runnability-logs\smoke-01.log`
...
```text
0001: # canonical-secret-lineage
0002: 
0003: Runtime token lineage with formatting changes and dedup-like history on main.
0004: 
```
...
### `app/security/rotation_state.py`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business module contributing to Rotation State. It is executable/live in the assembled repository.
...
### `docs/examples/runtime-reference.md`

- Why this file matters: `docs` file with expectation `must_not_find`.
- Detailed summary: Documentation or explanatory material for Runtime Reference. It is non-live or protected in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`False`; protected=`True`.
- Representative excerpt:

```text
0001: PARTNER_RUNTIME_TOKEN=example-value
```

### `ops/archive/partner_runtime.env`

- Why this file matters: `live-config` file with expectation `must_find`.
- Detailed summary: Runtime configuration carrying environment or deployment settings for Partner Runtime. It is executable/live in the assembled repository.
...
- path: `docs/examples/runtime-reference.md`
...
- path: `app/security/rotation_state.py`
  why finding must not appear: scenario declares this path clean despite adversarial content or naming.
