# Generated Project Dossier - Q-V6-TS-001

## 1. Scenario Identity

- scenarioId: `Q-V6-TS-001`
- scenarioName: Quantum TLS trust bypass patterns in Node HTTP clients.
- milestone: `M6`
- targetModule: `Quantum`
- language / stack: `TypeScript / Node`
- repoType: `snapshot`
- repo local path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\customer-portal-node-tls`
- repo remote provider: `github`
- repo remote URL: `https://github.com/ARQ-Sec/customer-portal-node-tls`
- default branch: `main`
- scan modes intended for this scenario: `HEAD_SNAPSHOT`
- branch scopes intended for this scenario: `SINGLE_BRANCH`
- project-local dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\customer-portal-node-tls\validation\generated-project-dossier.md`
- required alias dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\Q-V6-TS-001\validation\generated-project-dossier.md`
- central dossier report path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\reports\generated-project-dossiers\Q-V6-TS-001.md`

## 2. Business Context and Why This Repo Is Realistic

This scenario models `Customer portal calling billing and profile APIs.` as a `TypeScript / Node` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Quantum TLS trust bypass patterns in Node HTTP clients.`.

## 3. Architecture Summary

- Major components: `__tests__, dist, docs, ops, scripts, sql, src, validation`
- Runtime role: `Customer portal calling billing and profile APIs.`
...
- Crypto/TLS flow if relevant: `insecureAgentFactory.ts, insecureBillingClient.ts`
...
|       |   |   `-- events.service.js
|       |   |-- http
|       |   |   |-- insecureAgentFactory.js
|       |   |   |-- insecureBillingClient.js
|       |   |   `-- secureAgentFactory.js
|       |   |-- invites
|       |   |   `-- invites.service.js
|       |   |-- partners
|       |   |   `-- partners.service.js
...
|   |   |   `-- events.service.ts
|   |   |-- http
|   |   |   |-- insecureAgentFactory.ts
|   |   |   |-- insecureBillingClient.ts
|   |   |   `-- secureAgentFactory.ts
|   |   |-- invites
|   |   |   `-- invites.service.ts
|   |   |-- partners
|   |   |   `-- partners.service.ts
...
| dist/src/modules/http/insecureAgentFactory.js | generated | 8 | Generated or derived project artifact related to Insecure Agent Factory. | no | no | yes | yes | no | no |
| dist/src/modules/http/insecureBillingClient.js | generated | 8 | Generated or derived project artifact related to Insecure Billing Client. | no | no | yes | yes | no | no |
| dist/src/modules/http/secureAgentFactory.js | generated | 8 | Generated or derived project artifact related to Secure Agent Factory. | no | no | yes | yes | no | no |
...
| validation/generated-project-dossier.md | generated | 837 | Machine-readable validation contract or generated audit artifact for this scenario. | no | no | yes | no | no | no |
...
- Path: `src/modules/http/insecureAgentFactory.ts`
  Why it should be detected: scenario declares `reject-unauthorized` as a live positive surface.
...
- Path: `src/modules/http/insecureBillingClient.ts`
  Why it should be detected: scenario declares `axios-insecure` as a live positive surface.
  Target module: `Quantum`
  Finding family / rule family expectation: `rejectUnauthorized`
  Head/history behavior: `head-only`
  Explainability expectation: resolvedValue~rejectUnauthorized
...
| dist/src/modules/http/insecureAgentFactory.js | generated | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
| dist/src/modules/http/insecureBillingClient.js | generated | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
| dist/src/modules/http/secureAgentFactory.js | generated | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
...
- Command: `C:\Program Files\nodejs\npm.cmd install --silent`
  ...
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\customer-portal-node-tls\validation\runnability-logs\build-01.log`
- Command: `C:\Program Files\nodejs\npm.cmd run build`
  Expected result: `build` step completes successfully.
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\customer-portal-node-tls\validation\runnability-logs\build-02.log`
...
- Command: `C:\Program Files\nodejs\npm.cmd test -- --runInBand`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\customer-portal-node-tls\validation\runnability-logs\test-01.log`
...
- Command: `C:\Program Files\nodejs\node.exe dist/scripts/smoke.js`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\customer-portal-node-tls\validation\runnability-logs\smoke-01.log`
...
```text
0001: # customer-portal-node-tls
0002: 
0003: Customer portal calling billing and profile APIs.
0004: 
...
```text
0001: {
0002:   "name": "customer-portal-node-tls",
0003:   "version": "1.0.0",
0004:   "private": true,
...
### `src/modules/http/insecureAgentFactory.ts`
...
0002: export function buildAgent() { return new https.Agent({ rejectUnauthorized: false }); }
```

### `src/modules/http/insecureBillingClient.ts`
...
```text
0001: import https from 'https';
0002: export function billingAgent() { return new https.Agent({ rejectUnauthorized: false }); }
```

### `src/modules/http/secureAgentFactory.ts`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business module contributing to Secure Agent Factory. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: import https from 'https';
0002: export function buildSecureAgent() { return new https.Agent({ rejectUnauthorized: true }); }
```
...
- path: `insecureBillingClient.ts`
  module: `Quantum`
  expected rule/finding family: `rejectUnauthorized`
  expected branch/ref behavior: `head-only`
  expected explainability behavior: resolvedValue~rejectUnauthorized

### must_not_find

- path: `secureAgentFactory.ts`
...
- `insecureAgentFactory.ts`: resolvedValue~`rejectUnauthorized`, queryFamily~`typescript`
- `insecureBillingClient.ts`: resolvedValue~`rejectUnauthorized`
