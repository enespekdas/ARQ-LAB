# Generated Project Dossier - Q-V6-CONFIG-005

## 1. Scenario Identity

- scenarioId: `Q-V6-CONFIG-005`
- scenarioName: Quantum should keep active runtime TLS misconfig while ignoring dormant overlays and examples.
- milestone: `M6`
- targetModule: `Quantum`
- language / stack: `Config-heavy repo`
- repoType: `snapshot`
- repo local path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\live-overlay-config-matrix`
- repo remote provider: `github`
- repo remote URL: `https://github.com/ARQ-Sec/live-overlay-config-matrix`
- default branch: `main`
- scan modes intended for this scenario: `HEAD_SNAPSHOT`
- branch scopes intended for this scenario: `SINGLE_BRANCH`
- project-local dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\live-overlay-config-matrix\validation\generated-project-dossier.md`
- required alias dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\Q-V6-CONFIG-005\validation\generated-project-dossier.md`
- central dossier report path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\reports\generated-project-dossiers\Q-V6-CONFIG-005.md`

## 2. Business Context and Why This Repo Is Realistic

This scenario models `Runtime config repo separating live overlays from commented, example, and fixture-only insecure config.` as a `Config-heavy repo` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Quantum should keep active runtime TLS misconfig while ignoring dormant overlays and examples.`.

## 3. Architecture Summary

- Major components: `config, deploy, docs, examples, ops, runtime, scripts, sql, tests, validation`
- Runtime role: `Runtime config repo separating live overlays from commented, example, and fixture-only insecure config.`
- Config flow: `config/runtime/baseline.yaml, deploy/live/envoy.yaml, deploy/prod/service.properties, runtime/.env`
- Secret flow: `No Guardian must-find secret path in this scenario.`
- Crypto/TLS flow if relevant: `deploy/live/envoy.yaml, runtime/.env`
...
| deploy/live/envoy.yaml | live-config | 1 | Runtime configuration carrying environment or deployment settings for Envoy. | yes | no | no | yes | yes | no |
...
| docs/runtime-overlays.md | docs | 3 | Documentation or explanatory material for Runtime Overlays. | no | no | yes | no | no | no |
| examples/envoy/values.yaml | docs | 2 | Documentation or explanatory material for Values. | no | no | yes | no | no | no |
...
| runtime/.env | live-config | 1 | Runtime configuration carrying environment or deployment settings for .Env. | yes | no | no | yes | yes | no |
...
| validation/generated-project-dossier.md | generated | 714 | Machine-readable validation contract or generated audit artifact for this scenario. | no | no | yes | no | no | no |
...
- Path: `deploy/live/envoy.yaml`
  Why it should be detected: scenario declares `active-envoy-untrusted` as a live positive surface.
...
- Path: `runtime/.env`
  Why it should be detected: scenario declares `active-node-disable` as a live positive surface.
  Target module: `Quantum`
...
| docs/runtime-overlays.md | docs | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
| examples/envoy/values.yaml | docs | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
...
- Command: `C:\Python313\python.EXE scripts/validate_repo.py`
  ...
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\live-overlay-config-matrix\validation\runnability-logs\build-01.log`
...
- Command: `C:\Python313\python.EXE -m pytest -q`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\live-overlay-config-matrix\validation\runnability-logs\test-01.log`
...
- Command: `C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe -ExecutionPolicy Bypass -File scripts/smoke.ps1`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\live-overlay-config-matrix\validation\runnability-logs\smoke-01.log`
...
```text
0002: 
0003: Runtime config repo separating live overlays from commented, example, and fixture-only insecure config.
0004: 
```
...
### `docs/runtime-overlays.md`

- Why this file matters: `docs` file with expectation `must_not_find`.
- Detailed summary: Documentation or explanatory material for Runtime Overlays. It is non-live or protected in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`False`; protected=`True`.
- Representative excerpt:

```text
0001: # Example overlay
0002: # trust_chain_verification: ACCEPT_UNTRUSTED
0003: # NODE_TLS_REJECT_UNAUTHORIZED=0
```

### `examples/envoy/values.yaml`

- Why this file matters: `docs` file with expectation `must_not_find`.
- Detailed summary: Documentation or explanatory material for Values. It is non-live or protected in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`False`; protected=`True`.
- Representative excerpt:
...
- Why this file matters: `live-config` file with expectation `must_find`.
- Detailed summary: Runtime configuration carrying environment or deployment settings for .Env. It is executable/live in the assembled repository.
- Key constructs: positive surface; near-real=`False`; protected=`False`.
- Representative excerpt:
...
- Total LOC considered for authored/generated project content: `5010`
- Synthetic filler / inflation LOC: `4840`
- Synthetic filler ratio: `96.61%`
