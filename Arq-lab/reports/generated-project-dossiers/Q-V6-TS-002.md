# Generated Project Dossier - Q-V6-TS-002

## 1. Scenario Identity

- scenarioId: `Q-V6-TS-002`
- scenarioName: Quantum should preserve TypeScript TLS misuse semantics through returned wrapper and spread-based config flows.
- milestone: `M6`
- targetModule: `Quantum`
- language / stack: `TypeScript / Node`
- repoType: `snapshot`
- repo local path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-wrapper-node`
- repo remote provider: `github`
- repo remote URL: `https://github.com/ARQ-Sec/tls-wrapper-node`
- default branch: `main`
- scan modes intended for this scenario: `HEAD_SNAPSHOT`
- branch scopes intended for this scenario: `SINGLE_BRANCH`
- project-local dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-wrapper-node\validation\generated-project-dossier.md`
- required alias dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\Q-V6-TS-002\validation\generated-project-dossier.md`
- central dossier report path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\reports\generated-project-dossiers\Q-V6-TS-002.md`

## 2. Business Context and Why This Repo Is Realistic

This scenario models `Node client repo with rejectUnauthorized propagated through object spread and returned wrapper chains.` as a `TypeScript / Node` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Quantum should preserve TypeScript TLS misuse semantics through returned wrapper and spread-based config flows.`.

## 3. Architecture Summary

- Major components: `__tests__, dist, docs, ops, scripts, sql, src, validation`
- Runtime role: `Node client repo with rejectUnauthorized propagated through object spread and returned wrapper chains.`
...
- Crypto/TLS flow if relevant: `insecureSpreadAgent.ts`
...
|       |   |   `-- events.service.js
|       |   |-- http
|       |   |   |-- insecureSpreadAgent.js
|       |   |   `-- secureSpreadAgent.js
|       |   |-- invites
|       |   |   `-- invites.service.js
|       |   |-- partners
|       |   |   `-- partners.service.js
...
|   |   |   `-- events.service.ts
|   |   |-- http
|   |   |   |-- insecureSpreadAgent.ts
|   |   |   `-- secureSpreadAgent.ts
|   |   |-- invites
|   |   |   `-- invites.service.ts
|   |   |-- partners
|   |   |   `-- partners.service.ts
...
| dist/src/modules/http/insecureSpreadAgent.js | generated | 10 | Generated or derived project artifact related to Insecure Spread Agent. | no | no | yes | yes | no | no |
| dist/src/modules/http/secureSpreadAgent.js | generated | 10 | Generated or derived project artifact related to Secure Spread Agent. | no | no | yes | yes | no | no |
...
| docs/tls-wrapper.md | docs | 1 | Documentation or explanatory material for Tls Wrapper. | no | no | yes | no | no | no |
...
| src/modules/http/insecureSpreadAgent.ts | live-code | 4 | Runtime business module contributing to Insecure Spread Agent. | yes | yes | no | yes | yes | no |
| src/modules/http/secureSpreadAgent.ts | live-code | 4 | Runtime business module contributing to Secure Spread Agent. | no | yes | no | yes | yes | no |
...
| validation/generated-file-manifest.json | generated | 2718 | Machine-readable validation contract or generated audit artifact for this scenario. | no | no | yes | no | no | no |
| validation/generated-project-dossier.md | generated | 817 | Machine-readable validation contract or generated audit artifact for this scenario. | no | no | yes | no | no | no |
...
- Path: `src/modules/http/insecureSpreadAgent.ts`
  Why it should be detected: scenario declares `spread-reject-unauthorized` as a live positive surface.
  Target module: `Quantum`
  Finding family / rule family expectation: `rejectUnauthorized`
  Head/history behavior: `head-only`
  Explainability expectation: resolvedValue~rejectUnauthorized, queryFamily~typescript

## 7. Near-Real Negative Surfaces

- `src/modules/http/insecureSpreadAgent.ts`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
- `src/modules/http/secureSpreadAgent.ts`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
...
| dist/src/modules/http/insecureSpreadAgent.js | generated | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
| dist/src/modules/http/secureSpreadAgent.js | generated | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
...
| docs/tls-wrapper.md | docs | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
...
- Command: `C:\Program Files\nodejs\npm.cmd install --silent`
  ...
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-wrapper-node\validation\runnability-logs\build-01.log`
- Command: `C:\Program Files\nodejs\npm.cmd run build`
  Expected result: `build` step completes successfully.
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-wrapper-node\validation\runnability-logs\build-02.log`
...
- Command: `C:\Program Files\nodejs\npm.cmd test -- --runInBand`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-wrapper-node\validation\runnability-logs\test-01.log`
...
- Command: `C:\Program Files\nodejs\node.exe dist/scripts/smoke.js`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-wrapper-node\validation\runnability-logs\smoke-01.log`
...
```text
0002: 
0003: Node client repo with rejectUnauthorized propagated through object spread and returned wrapper chains.
0004: 
```
...
- Detailed summary: Documentation or explanatory material for Tls Wrapper. It is non-live or protected in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`False`; protected=`True`.
- Representative excerpt:

```text
0001: Wrapper and spread examples remain descriptive; only live runtime files should be flagged.
```
...
### `src/modules/http/insecureSpreadAgent.ts`

- Why this file matters: `live-code` file with expectation `must_find`.
- Detailed summary: Runtime business module contributing to Insecure Spread Agent. It is executable/live in the assembled repository.
- Key constructs: positive surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: import https from 'https';
0002: const sharedTls = { rejectUnauthorized: false };
0003: function buildTlsOptions() { return { ...sharedTls }; }
```

### `src/modules/http/secureSpreadAgent.ts`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business module contributing to Secure Spread Agent. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: import https from 'https';
0002: const sharedTls = { rejectUnauthorized: true };
0003: function buildTlsOptions() { return { ...sharedTls }; }
```

## 12. Line Composition and Filler Disclosure

- Total LOC considered for authored/generated project content: `9926`
- Synthetic filler / inflation LOC: `4840`
- Synthetic filler ratio: `48.76%`

| category | LOC |
| --- | ---: |
| live business code | 28 |
| live config | 0 |
| tests | 16 |
| docs | 12 |
| scripts | 1 |
| fixtures | 0 |
| vendor/generated | 238 |
...
- path: `insecureSpreadAgent.ts`
  module: `Quantum`
  expected rule/finding family: `rejectUnauthorized`
  expected branch/ref behavior: `head-only`
  expected explainability behavior: resolvedValue~rejectUnauthorized, queryFamily~typescript

### must_not_find

- path: `secureSpreadAgent.ts`
...
- `insecureSpreadAgent.ts`: resolvedValue~`rejectUnauthorized`, queryFamily~`typescript`
