# Generated Project Dossier - Q-V4-PY-005

## 1. Scenario Identity

- scenarioId: `Q-V4-PY-005`
- scenarioName: Quantum should distinguish Python crypto inventory from actionable hashlib misuse.
- milestone: `M4`
- targetModule: `Quantum`
- language / stack: `Python / service helper library`
- repoType: `snapshot`
- repo local path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-usage-python`
- repo remote provider: `github`
- repo remote URL: `https://github.com/ARQ-Sec/crypto-usage-python`
- default branch: `main`
- scan modes intended for this scenario: `HEAD_SNAPSHOT`
- branch scopes intended for this scenario: `SINGLE_BRANCH`
- project-local dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-usage-python\validation\generated-project-dossier.md`
- required alias dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\Q-V4-PY-005\validation\generated-project-dossier.md`
- central dossier report path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\reports\generated-project-dossiers\Q-V4-PY-005.md`

## 2. Business Context and Why This Repo Is Realistic

This scenario models `Python utility repo with one real md5 path and several safe import-only files.` as a `Python / service helper library` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Quantum should distinguish Python crypto inventory from actionable hashlib misuse.`.

## 3. Architecture Summary

- Major components: `app, docs, ops, scripts, sql, tests, validation`
- Runtime role: `Python utility repo with one real md5 path and several safe import-only files.`
...
- Crypto/TLS flow if relevant: `legacy_signature.py`
...
- Path: `app/security/legacy_signature.py`
  Why it should be detected: scenario declares `md5-signature` as a live positive surface.
...
- Command: `C:\Python313\python.EXE -m compileall app tests scripts`
  ...
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-usage-python\validation\runnability-logs\build-01.log`
...
- Command: `C:\Python313\python.EXE -m pytest -q`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-usage-python\validation\runnability-logs\test-01.log`
...
- Command: `C:\Python313\python.EXE scripts/smoke.py`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-usage-python\validation\runnability-logs\smoke-01.log`
...
```text
0002: 
0003: Python utility repo with one real md5 path and several safe import-only files.
0004: 
```
...
### `app/security/secure_hash.py`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business module contributing to Secure Hash. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:
...
```text
0001: Use hashlib.sha256 in runtime code; examples remain descriptive only.
```
