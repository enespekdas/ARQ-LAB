# Generated Project Dossier - G-V1-PY-002

## 1. Scenario Identity

- scenarioId: `G-V1-PY-002`
- scenarioName: Guardian live secret surfaces in a realistic Python notification worker.
- milestone: `M1`
...
- repo remote URL: `https://github.com/ARQ-Sec/notification-worker-python`
- default branch: `main`
...
- Docs/vendor/generated surfaces: `.env.example, README.md, docs/architecture/section-01.md, docs/architecture/section-02.md, docs/architecture/section-03.md, docs/architecture/section-04.md, docs/architecture/section-05.md, docs/architecture/section-06.md, docs/architecture/section-07.md, docs/architecture/section-08.md, docs/architecture/section-09.md, docs/architecture/section-10.md`
...
| --- | --- | ---: | --- | --- | --- | --- | --- | --- | --- |
| .env.example | generated | 1 | Generated or derived project artifact related to .Env. | no | no | yes | no | no | no |
...
| app/clients/webhook_client.py | live-code | 1 | Outbound integration client for Webhook Client behavior. | yes | no | no | yes | yes | no |
| app/config/runtime_settings.py | live-code | 1 | Runtime business module contributing to Runtime Settings. | yes | no | no | yes | yes | no |
...
| app/security/jwt_bootstrap.py | live-code | 1 | Runtime business module contributing to Jwt Bootstrap. | yes | no | no | yes | yes | no |
...
| deploy/k8s/notification-prod.env | live-config | 1 | Runtime configuration carrying environment or deployment settings for Notification Prod. | yes | no | no | yes | yes | no |
...
| validation/generated-project-dossier.md | generated | 798 | Machine-readable validation contract or generated audit artifact for this scenario. | no | no | yes | no | no | no |
...
- Path: `app/config/runtime_settings.py`
  Why it should be detected: scenario declares `runtime-token` as a live positive surface.
...
- Path: `deploy/k8s/notification-prod.env`
  Why it should be detected: scenario declares `prod-env` as a live positive surface.
  Target module: `Guardian`
  Finding family / rule family expectation: `guardian.generic-api-key`
  Head/history behavior: `head-only`
  Explainability expectation: No strict scenario-specific explainability contract beyond normal detail capture.
- Path: `app/clients/webhook_client.py`
  Why it should be detected: scenario declares `webhook-key` as a live positive surface.
  Target module: `Guardian`
  Finding family / rule family expectation: `guardian.generic-api-key`
  Head/history behavior: `head-only`
  Explainability expectation: No strict scenario-specific explainability contract beyond normal detail capture.
- Path: `app/security/jwt_bootstrap.py`
  Why it should be detected: scenario declares `jwt-secret` as a live positive surface.
  Target module: `Guardian`
  Finding family / rule family expectation: `guardian.generic-api-key`
  Head/history behavior: `head-only`
  Explainability expectation: No strict scenario-specific explainability contract beyond normal detail capture.
...
| --- | --- | --- |
| .env.example | generated | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
...
- Detailed summary: Generated or derived project artifact related to .Env. It is non-live or protected in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`False`; protected=`True`.
- Representative excerpt:

```text
0001: JWT_SECRET=example-value
```
...
```text
0001: # notification-worker-python
0002: 
0003: Notification dispatch and webhook worker.
0004: 
...
### `app/clients/webhook_client.py`

- Why this file matters: `live-code` file with expectation `must_find`.
- Detailed summary: Outbound integration client for Webhook Client behavior. It is executable/live in the assembled repository.
- Key constructs: positive surface; near-real=`False`; protected=`False`.
- Representative excerpt:

```text
0001: WEBHOOK_TOKEN = "J8nP3vT6xZ1mR4qW7cF2hK9sL5dY8b"
```

### `app/config/runtime_settings.py`

- Why this file matters: `live-code` file with expectation `must_find`.
- Detailed summary: Runtime business module contributing to Runtime Settings. It is executable/live in the assembled repository.
- Key constructs: positive surface; near-real=`False`; protected=`False`.
- Representative excerpt:

```text
0001: PROVIDER_TOKEN = "A3-ABC123-ABCDEFGHIJK-ABCDE-ABCDE-ABCDE"
```
...
### `app/security/jwt_bootstrap.py`

- Why this file matters: `live-code` file with expectation `must_find`.
- Detailed summary: Runtime business module contributing to Jwt Bootstrap. It is executable/live in the assembled repository.
- Key constructs: positive surface; near-real=`False`; protected=`False`.
- Representative excerpt:

```text
0001: JWT_SECRET = "0123456789ABCDEFA31a"
```

### `deploy/k8s/notification-prod.env`

- Why this file matters: `live-config` file with expectation `must_find`.
- Detailed summary: Runtime configuration carrying environment or deployment settings for Notification Prod. It is executable/live in the assembled repository.
- Key constructs: positive surface; near-real=`False`; protected=`False`.
- Representative excerpt:

```text
0001: NOTIFICATION_SECRET=Ab9K2mQ7pL4xR8nT5vW1Yz7Ck3Hs6Fq2
```
...
- Synthetic filler ratio: `93.52%`

| category | LOC |
| --- | ---: |
| live business code | 88 |
| live config | 1 |
| tests | 32 |
| docs | 11 |
| scripts | 1 |
