# Generated Project Dossier - G-V2-HIST-016

## 1. Scenario Identity

- scenarioId: `G-V2-HIST-016`
- scenarioName: Guardian canonicalization and history handling for multiline YAML folded secret lineage.
- milestone: `M2`
- targetModule: `Guardian`
- language / stack: `Python + YAML ops`
- repoType: `history`
- repo local path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\folded-yaml-lineage`
- repo remote provider: `github`
- repo remote URL: `https://github.com/ARQ-Sec/folded-yaml-lineage`
- default branch: `main`
- scan modes intended for this scenario: `HEAD_SNAPSHOT, REF_HISTORY`
- branch scopes intended for this scenario: `SINGLE_BRANCH`
- project-local dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\folded-yaml-lineage\validation\generated-project-dossier.md`
- required alias dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\G-V2-HIST-016\validation\generated-project-dossier.md`
- central dossier report path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\reports\generated-project-dossiers\G-V2-HIST-016.md`

## 2. Business Context and Why This Repo Is Realistic

This scenario models `Runtime config repo where a folded YAML scalar once held a live token and later became a masked docs example.` as a `Python + YAML ops` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Guardian canonicalization and history handling for multiline YAML folded secret lineage.`.

## 3. Architecture Summary

- Major components: `app, docs, ops, scripts, sql, tests, validation`
- Runtime role: `Runtime config repo where a folded YAML scalar once held a live token and later became a masked docs example.`
- Config flow: `ops/runtime/runtime-values.yaml`
- Secret flow: `runtime-values.yaml`
...
| docs/masked-values.yaml | docs | 2 | Documentation or explanatory material for Masked Values. | no | no | yes | no | no | no |
| docs/tutorials/runtime-values.md | docs | 1 | Documentation or explanatory material for Runtime Values. | no | no | yes | no | no | no |
...
| ops/runtime/runtime-values.yaml | live-config | 2 | Runtime configuration carrying environment or deployment settings for Runtime Values. | yes | no | no | yes | yes | no |
...
- Path: `ops/runtime/runtime-values.yaml`
  Why it should be detected: scenario declares `folded-yaml-secret` as a live positive surface.
...
| docs/masked-values.yaml | docs | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
| docs/tutorials/runtime-values.md | docs | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
...
- `main` tip: `64d7219237e6255bd087dea32077dfc6d8e027aa`

Commit order:

- `0920b16526212d40b52eb9878d898e9eb99e3e79` `c001 bootstrap`: initial clean or baseline assembly.
- `ed02d3053e31bfcdb63db254bcbbd60cf73e040e` `c002 add folded yaml secret`: introduces an intended signal.
- `ea4f838979759a32d6add362bc7a846a3201efd8` `c003 add masked docs example`: introduces an intended signal.
- `64d7219237e6255bd087dea32077dfc6d8e027aa` `c004 remove live secret from head`: removes or neutralizes a prior signal.

Expected final head/history state:

- `folded-yaml-secret` on `runtime-values.yaml` should behave as `HISTORY_ONLY`.
...
- Command: `C:\Python313\python.EXE -m compileall app tests scripts`
  ...
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\folded-yaml-lineage\validation\runnability-logs\build-01.log`
...
- Command: `C:\Python313\python.EXE -m pytest -q`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\folded-yaml-lineage\validation\runnability-logs\test-01.log`
...
- Command: `C:\Python313\python.EXE scripts/smoke.py`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\folded-yaml-lineage\validation\runnability-logs\smoke-01.log`
...
```text
0002: 
0003: Runtime config repo where a folded YAML scalar once held a live token and later became a masked docs example.
0004: 
```
...
### `docs/masked-values.yaml`

- Why this file matters: `docs` file with expectation `must_not_find`.
- Detailed summary: Documentation or explanatory material for Masked Values. It is non-live or protected in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`False`; protected=`True`.
- Representative excerpt:

```text
0001: runtime:
0002:   partnerToken: masked-example-value
```

### `docs/tutorials/runtime-values.md`

- Why this file matters: `docs` file with expectation `must_not_find`.
- Detailed summary: Documentation or explanatory material for Runtime Values. It is non-live or protected in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`False`; protected=`True`.
- Representative excerpt:

```text
0001: Folded YAML examples in docs remain placeholder-only.
```

### `ops/runtime/runtime-values.yaml`

- Why this file matters: `live-config` file with expectation `must_find`.
- Detailed summary: Runtime configuration carrying environment or deployment settings for Runtime Values. It is executable/live in the assembled repository.
- Key constructs: positive surface; near-real=`False`; protected=`False`.
- Representative excerpt:

```text
0001: runtime:
0002:   partnerToken: masked-reference-only
```
...
- Total LOC considered for authored/generated project content: `4152`
- Synthetic filler / inflation LOC: `3850`
- Synthetic filler ratio: `92.73%`
...
- path: `docs/tutorials/runtime-values.md`
  why finding must not appear: scenario declares this path clean despite adversarial content or naming.
  negative type: `protected negative`
