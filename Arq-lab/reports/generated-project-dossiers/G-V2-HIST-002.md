# Generated Project Dossier - G-V2-HIST-002

## 1. Scenario Identity

- scenarioId: `G-V2-HIST-002`
- scenarioName: Guardian single-branch versus all-branches branch divergence handling.
- milestone: `M2`
...
- repo remote URL in Gitea: `http://localhost:3001/arq/partner-branch-divergence-20260330t214509z`
- default branch: `main`
...
This scenario models `Branch divergence with main clean and feature branch leaked.` as a `TypeScript / Node` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Guardian single-branch versus all-branches branch divergence handling.`.

## 3. Architecture Summary

- Major components: `__tests__, dist, docs, ops, scripts, sql, src, validation`
- Runtime role: `Branch divergence with main clean and feature branch leaked.`
- Config flow: `No dedicated live-config files; runtime behavior is code-driven.`
- Secret flow: `src/modules/partner/bootstrap/branchLeak.ts`
...
| src/modules/partner/bootstrap/branchLeak.ts | live-code | 1 | Runtime business module contributing to Branch Leak. | yes | no | no | yes | yes | no |
...
- Path: `src/modules/partner/bootstrap/branchLeak.ts`
  Why it should be detected: scenario declares `feature-branch-secret` as a live positive surface.
...
- `feature/docs-refresh` tip: `ba235638bcfad263c052d2cabde66276d15c87e1`; diverges from `main` at `0d0befabbe44a9daf13f4a124897437f727fdcf3`
- `feature/partner-hotfix` tip: `5fb2f110551a4894ed284cccf7a6ac2549147eb2`; diverges from `main` at `0d0befabbe44a9daf13f4a124897437f727fdcf3`
- `main` tip: `0d0befabbe44a9daf13f4a124897437f727fdcf3`
- `release/2026.04` tip: `ee5b7a06ebab06a3a50bfc44f742deba1998a3bd`; diverges from `main` at `0d0befabbe44a9daf13f4a124897437f727fdcf3`

Commit order:

- `0d0befabbe44a9daf13f4a124897437f727fdcf3` `c001 main clean baseline`: removes or neutralizes a prior signal.
- `ee5b7a06ebab06a3a50bfc44f742deba1998a3bd` `c003 release branch clean state`: removes or neutralizes a prior signal.
- `5fb2f110551a4894ed284cccf7a6ac2549147eb2` `c002 temporary secret on feature branch`: introduces an intended signal.
- `ba235638bcfad263c052d2cabde66276d15c87e1` `c004 noise-only docs branch`: adds documentation-only or noise-only collateral.

Expected final head/history state:

- `feature-branch-secret` on `src/modules/partner/bootstrap/branchLeak.ts` should behave as `head-only`.
...
```text
0001: # partner-branch-divergence
0002: 
0003: Branch divergence with main clean and feature branch leaked.
0004: 
...
```text
0001: {
0002:   "name": "partner-branch-divergence",
0003:   "version": "1.0.0",
0004:   "private": true,
...
### `src/modules/partner/bootstrap/branchLeak.ts`

- Why this file matters: `live-code` file with expectation `must_find`.
- Detailed summary: Runtime business module contributing to Branch Leak. It is executable/live in the assembled repository.
- Key constructs: positive surface; near-real=`False`; protected=`False`.
- Representative excerpt:

```text
0001: export const featureBranchToken = 'placeholder-clean-value';
```
...
- path: `src/modules/partner/bootstrap/mainSafe.ts`
