# Generated Project Dossier - Q-V4-PY-006

## 1. Scenario Identity

- scenarioId: `Q-V4-PY-006`
- scenarioName: Quantum should keep the live md5 path while leaving import-only and safe wrapper files clean in Python.
- milestone: `M4`
- targetModule: `Quantum`
- language / stack: `Python / service helper library`
- repoType: `snapshot`
- repo local path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\hash-wrapper-python`
- repo remote provider: `github`
- repo remote URL: `https://github.com/ARQ-Sec/hash-wrapper-python`
- default branch: `main`
- scan modes intended for this scenario: `HEAD_SNAPSHOT`
- branch scopes intended for this scenario: `SINGLE_BRANCH`
- project-local dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\hash-wrapper-python\validation\generated-project-dossier.md`
- required alias dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\Q-V4-PY-006\validation\generated-project-dossier.md`
- central dossier report path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\reports\generated-project-dossiers\Q-V4-PY-006.md`

## 2. Business Context and Why This Repo Is Realistic

This scenario models `Python hash helper repo using aliases and wrappers with one real live md5 path nearby import-only traps.` as a `Python / service helper library` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Quantum should keep the live md5 path while leaving import-only and safe wrapper files clean in Python.`.

## 3. Architecture Summary

- Major components: `app, docs, ops, scripts, sql, tests, validation`
- Runtime role: `Python hash helper repo using aliases and wrappers with one real live md5 path nearby import-only traps.`
...
- Crypto/TLS flow if relevant: `live_md5.py`
...
|   |   `-- templates.py
|   |-- security
|   |   |-- hash_alias_inventory.py
|   |   |-- live_md5.py
|   |   `-- safe_hash_wrapper.py
|   |-- services
|   |   |-- audit_service.py
|   |   |-- delivery_service.py
|   |   |-- dispatch_service.py
...
| app/security/hash_alias_inventory.py | live-code | 4 | Runtime business module contributing to Hash Alias Inventory. | no | yes | no | yes | yes | no |
| app/security/live_md5.py | live-code | 4 | Runtime business module contributing to Live Md5. | yes | no | no | yes | yes | no |
| app/security/safe_hash_wrapper.py | live-code | 4 | Runtime business module contributing to Safe Hash Wrapper. | no | yes | no | yes | yes | no |
...
- Path: `app/security/live_md5.py`
  Why it should be detected: scenario declares `live-md5` as a live positive surface.
  Target module: `Quantum`
  Finding family / rule family expectation: `md5`
  Head/history behavior: `head-only`
  Explainability expectation: resolvedValue~md5, queryFamily~python

## 7. Near-Real Negative Surfaces

- `app/security/hash_alias_inventory.py`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
- `app/security/safe_hash_wrapper.py`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
...
- Command: `C:\Python313\python.EXE -m compileall app tests scripts`
  ...
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\hash-wrapper-python\validation\runnability-logs\build-01.log`
...
- Command: `C:\Python313\python.EXE -m pytest -q`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\hash-wrapper-python\validation\runnability-logs\test-01.log`
...
- Command: `C:\Python313\python.EXE scripts/smoke.py`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\hash-wrapper-python\validation\runnability-logs\smoke-01.log`
...
```text
0002: 
0003: Python hash helper repo using aliases and wrappers with one real live md5 path nearby import-only traps.
0004: 
```
...
### `app/security/hash_alias_inventory.py`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business module contributing to Hash Alias Inventory. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: import hashlib as digest_lib
0002: 
0003: def supported_algorithms() -> tuple[str, ...]:
0004:     return ('sha256', digest_lib.sha512().name)
```

### `app/security/live_md5.py`

- Why this file matters: `live-code` file with expectation `must_find`.
- Detailed summary: Runtime business module contributing to Live Md5. It is executable/live in the assembled repository.
- Key constructs: positive surface; near-real=`False`; protected=`False`.
- Representative excerpt:

```text
0003: def sign(payload: bytes) -> str:
0004:     return digest_lib.md5(payload).hexdigest()
```

### `app/security/safe_hash_wrapper.py`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business module contributing to Safe Hash Wrapper. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:
...
0002: 
0003: def sign(payload: bytes) -> str:
0004:     return hashlib.sha256(payload).hexdigest()
```
...
- Total LOC considered for authored/generated project content: `5028`
- Synthetic filler / inflation LOC: `4730`
- Synthetic filler ratio: `94.07%`

| category | LOC |
| --- | ---: |
| live business code | 97 |
| live config | 0 |
| tests | 32 |
| docs | 11 |
| scripts | 1 |
...
- path: `live_md5.py`
  ...
  expected explainability behavior: resolvedValue~md5, queryFamily~python

### must_not_find

- path: `hash_alias_inventory.py`
...
- `live_md5.py`: resolvedValue~`md5`, queryFamily~`python`
