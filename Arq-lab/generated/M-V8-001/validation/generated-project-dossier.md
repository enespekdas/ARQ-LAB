# Generated Project Dossier - M-V8-001

## 1. Scenario Identity

- scenarioId: `M-V8-001`
- scenarioName: Mixed enterprise monorepo with both Guardian and Quantum signals.
- milestone: `M8`
- targetModule: `Both`
...
- repo remote URL in Gitea: `http://localhost:3001/arq/internal-platform-monorepo-20260330t135123z`
- default branch: `main`
...
This scenario models `Internal platform monorepo with multiple tech stacks.` as a `Java + Node + Python monorepo` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Mixed enterprise monorepo with both Guardian and Quantum signals.`.

## 3. Architecture Summary

- Major components: `apps, docs, ops, scripts, services, shared, sql, validation, workers`
- Runtime role: `Internal platform monorepo with multiple tech stacks.`
- Config flow: `services/platform-java/src/main/resources/application-prod.yml, services/platform-java/src/main/resources/application.yml`
- Secret flow: `application-prod.yml, platform_service.pem, runtime_secret.py`
- Crypto/TLS flow if relevant: `LegacyCipherService.java, insecure_partner.py, legacyDigest.ts`
...
| shared/config/keys/platform_service.pem | generated | 10 | Generated or derived project artifact related to Platform Service. | yes | no | yes | no | no | no |
...
| validation/expected-findings.json | generated | 80 | Machine-readable validation contract or generated audit artifact for this scenario. | no | no | yes | no | no | no |
...
| workers/sync-worker/app/security/runtime_secret.py | live-code | 1 | Runtime business module contributing to Runtime Secret. | yes | no | no | yes | yes | no |
...
| workers/sync-worker/tests/test_audit_service.py | live-code | 4 | Runtime business service implementing Test Audit Service logic. | no | yes | no | yes | yes | no |
| workers/sync-worker/tests/test_delivery_service.py | live-code | 4 | Runtime business service implementing Test Delivery Service logic. | no | yes | no | yes | yes | no |
| workers/sync-worker/tests/test_dispatch_service.py | live-code | 4 | Runtime business service implementing Test Dispatch Service logic. | no | yes | no | yes | yes | no |
| workers/sync-worker/tests/test_metrics_service.py | live-code | 4 | Runtime business service implementing Test Metrics Service logic. | no | yes | no | yes | yes | no |
| workers/sync-worker/tests/test_preferences_service.py | live-code | 4 | Runtime business service implementing Test Preferences Service logic. | no | yes | no | yes | yes | no |
| workers/sync-worker/tests/test_retry_service.py | live-code | 4 | Runtime business service implementing Test Retry Service logic. | no | yes | no | yes | yes | no |
| workers/sync-worker/tests/test_routing_service.py | live-code | 4 | Runtime business service implementing Test Routing Service logic. | no | yes | no | yes | yes | no |
| workers/sync-worker/tests/test_templates_service.py | live-code | 4 | Runtime business service implementing Test Templates Service logic. | no | yes | no | yes | yes | no |
...
- Path: `workers/sync-worker/app/security/runtime_secret.py`
  Why it should be detected: scenario declares `guardian-python-key` as a live positive surface.
  Target module: `Guardian`
  Finding family / rule family expectation: `guardian.generic-api-key`
  Head/history behavior: `head-only`
  Explainability expectation: No strict scenario-specific explainability contract beyond normal detail capture.
- Path: `shared/config/keys/platform_service.pem`
  ...
  Head/history behavior: `head-only`
  Explainability expectation: No strict scenario-specific explainability contract beyond normal detail capture.
- Path: `services/platform-java/src/main/java/com/arq/platformjava/security/LegacyCipherService.java`
  Why it should be detected: scenario declares `quantum-java-ecb` as a live positive surface.
...
- `workers/sync-worker/tests/test_metrics_service.py`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
- `workers/sync-worker/tests/test_preferences_service.py`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
- `workers/sync-worker/tests/test_retry_service.py`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
- `workers/sync-worker/tests/test_routing_service.py`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
- `workers/sync-worker/tests/test_templates_service.py`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
...
- `main` tip: `72e934e0a5b67df5ab00413d8a8402b5d235ea68`

Commit order:

- `72e934e0a5b67df5ab00413d8a8402b5d235ea68` `bootstrap M-V8-001`: initial clean or baseline assembly.

Expected final head/history state:

- `guardian-java-secret` on `application-prod.yml` should behave as `head-only`.
- `guardian-python-key` on `runtime_secret.py` should behave as `head-only`.
- `guardian-private-key` on `platform_service.pem` should behave as `head-only`.
- `quantum-java-ecb` on `LegacyCipherService.java` should behave as `head-only`.
...
```text
0001: # internal-platform-monorepo
0002: 
0003: Internal platform monorepo with multiple tech stacks.
0004: 
...
### `workers/sync-worker/tests/test_audit_service.py`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business service implementing Test Audit Service logic. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: from app.services.audit_service import load_audit_records
0002: 
0003: def test_audit_records_present() -> None:
0004:     assert load_audit_records()
```

### `workers/sync-worker/tests/test_delivery_service.py`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business service implementing Test Delivery Service logic. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: from app.services.delivery_service import load_delivery_records
0002: 
0003: def test_delivery_records_present() -> None:
0004:     assert load_delivery_records()
```

### `workers/sync-worker/tests/test_dispatch_service.py`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business service implementing Test Dispatch Service logic. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: from app.services.dispatch_service import load_dispatch_records
0002: 
0003: def test_dispatch_records_present() -> None:
0004:     assert load_dispatch_records()
```

### `workers/sync-worker/tests/test_metrics_service.py`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business service implementing Test Metrics Service logic. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: from app.services.metrics_service import load_metrics_records
0002: 
0003: def test_metrics_records_present() -> None:
0004:     assert load_metrics_records()
```

### `workers/sync-worker/tests/test_preferences_service.py`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business service implementing Test Preferences Service logic. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: from app.services.preferences_service import load_preferences_records
0002: 
0003: def test_preferences_records_present() -> None:
0004:     assert load_preferences_records()
```

### `workers/sync-worker/tests/test_retry_service.py`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business service implementing Test Retry Service logic. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: from app.services.retry_service import load_retry_records
0002: 
0003: def test_retry_records_present() -> None:
0004:     assert load_retry_records()
```

### `workers/sync-worker/tests/test_routing_service.py`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business service implementing Test Routing Service logic. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: from app.services.routing_service import load_routing_records
0002: 
0003: def test_routing_records_present() -> None:
0004:     assert load_routing_records()
```

### `workers/sync-worker/tests/test_templates_service.py`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business service implementing Test Templates Service logic. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: from app.services.templates_service import load_templates_records
0002: 
0003: def test_templates_records_present() -> None:
0004:     assert load_templates_records()
```
...
- path: `runtime_secret.py`
  module: `Guardian`
  expected rule/finding family: `guardian.generic-api-key`
  expected branch/ref behavior: `head-only`
  expected explainability behavior: No strict scenario-specific explainability contract beyond normal detail capture.
- path: `platform_service.pem`
  module: `Guardian`
  expected rule/finding family: `guardian.private-key`
  expected branch/ref behavior: `head-only`
  expected explainability behavior: No strict scenario-specific explainability contract beyond normal detail capture.
- path: `LegacyCipherService.java`
  module: `Quantum`
...
- path: `docs/examples`
  why finding must not appear: scenario declares this path clean despite adversarial content or naming.
  negative type: `near-real negative`
- path: `workers/sync-worker/tests`
  why finding must not appear: scenario declares this path clean despite adversarial content or naming.
  negative type: `near-real negative`
...
- `LegacyCipherService.java`: resolvedValue~`AES/ECB/PKCS5Padding`
