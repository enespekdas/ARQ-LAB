# Generated Project Dossier - Q-V4-TS-006

## 1. Scenario Identity

- scenarioId: `Q-V4-TS-006`
- scenarioName: Quantum should follow destructuring and returned object chains to the live createHash misuse in TypeScript.
- milestone: `M4`
- targetModule: `Quantum`
- language / stack: `TypeScript / Node`
- repoType: `snapshot`
- repo local path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\hash-destructure-node`
- repo remote provider: `github`
- repo remote URL: `https://github.com/ARQ-Sec/hash-destructure-node`
- default branch: `main`
- scan modes intended for this scenario: `HEAD_SNAPSHOT`
- branch scopes intended for this scenario: `SINGLE_BRANCH`
- project-local dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\hash-destructure-node\validation\generated-project-dossier.md`
- required alias dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\Q-V4-TS-006\validation\generated-project-dossier.md`
- central dossier report path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\reports\generated-project-dossiers\Q-V4-TS-006.md`

## 2. Business Context and Why This Repo Is Realistic

This scenario models `Node repo using destructured crypto helpers, alias exports, and returned object chains around createHash.` as a `TypeScript / Node` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Quantum should follow destructuring and returned object chains to the live createHash misuse in TypeScript.`.

## 3. Architecture Summary

- Major components: `__tests__, dist, docs, ops, scripts, sql, src, validation`
- Runtime role: `Node repo using destructured crypto helpers, alias exports, and returned object chains around createHash.`
...
- Crypto/TLS flow if relevant: `hashComposer.ts`
...
| dist/src/modules/security/hashCatalog.js | generated | 4 | Generated or derived project artifact related to Hash Catalog. | no | no | yes | yes | no | no |
| dist/src/modules/security/hashComposer.js | generated | 40 | Generated or derived project artifact related to Hash Composer. | no | no | yes | yes | no | no |
| dist/src/modules/security/safeHashComposer.js | generated | 39 | Generated or derived project artifact related to Safe Hash Composer. | no | no | yes | yes | no | no |
...
| src/modules/security/hashCatalog.ts | live-code | 1 | Runtime business module contributing to Hash Catalog. | no | yes | no | yes | yes | no |
| src/modules/security/hashComposer.ts | live-code | 4 | Runtime business module contributing to Hash Composer. | yes | yes | no | yes | yes | no |
| src/modules/security/safeHashComposer.ts | live-code | 3 | Runtime business module contributing to Safe Hash Composer. | no | yes | no | yes | yes | no |
...
- Path: `src/modules/security/hashComposer.ts`
  Why it should be detected: scenario declares `destructured-md5` as a live positive surface.
...
- `src/modules/security/hashCatalog.ts`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
- `src/modules/security/hashComposer.ts`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
- `src/modules/security/safeHashComposer.ts`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
...
| dist/src/modules/security/hashCatalog.js | generated | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
| dist/src/modules/security/hashComposer.js | generated | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
| dist/src/modules/security/safeHashComposer.js | generated | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
...
- Command: `C:\Program Files\nodejs\npm.cmd install --silent`
  ...
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\hash-destructure-node\validation\runnability-logs\build-01.log`
- Command: `C:\Program Files\nodejs\npm.cmd run build`
  Expected result: `build` step completes successfully.
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\hash-destructure-node\validation\runnability-logs\build-02.log`
...
- Command: `C:\Program Files\nodejs\npm.cmd test -- --runInBand`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\hash-destructure-node\validation\runnability-logs\test-01.log`
...
- Command: `C:\Program Files\nodejs\node.exe dist/scripts/smoke.js`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\hash-destructure-node\validation\runnability-logs\smoke-01.log`
...
```text
0001: # hash-destructure-node
0002: 
0003: Node repo using destructured crypto helpers, alias exports, and returned object chains around createHash.
0004: 
...
```text
0001: {
0002:   "name": "hash-destructure-node",
0003:   "version": "1.0.0",
0004:   "private": true,
...
### `src/modules/security/hashCatalog.ts`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business module contributing to Hash Catalog. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: export const supportedHashes = ['sha256', 'sha512'];
```

### `src/modules/security/hashComposer.ts`

- Why this file matters: `live-code` file with expectation `must_find`.
- Detailed summary: Runtime business module contributing to Hash Composer. It is executable/live in the assembled repository.
- Key constructs: positive surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0002: const { createHash: createLegacyHash } = crypto;
0003: function legacyFactory() { return { build: () => createLegacyHash('md5') }; }
0004: export function digest(value: string): string { return legacyFactory().build().update(value).digest('hex'); }
```

### `src/modules/security/safeHashComposer.ts`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business module contributing to Safe Hash Composer. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: import * as crypto from 'crypto';
0002: const { createHash } = crypto;
0003: export function digest(value: string): string { return createHash('sha256').update(value).digest('hex'); }
```

## 12. Line Composition and Filler Disclosure

- Total LOC considered for authored/generated project content: `9988`
- Synthetic filler / inflation LOC: `4840`
- Synthetic filler ratio: `48.46%`
...
- `hashComposer.ts`: resolvedValue~`MD5`, queryFamily~`typescript`
