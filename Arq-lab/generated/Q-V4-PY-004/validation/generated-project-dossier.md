# Generated Project Dossier - Q-V4-PY-004

## 1. Scenario Identity

- scenarioId: `Q-V4-PY-004`
- scenarioName: Quantum precision for Python wrapper-safe surfaces versus actionable weak RNG calls.
- milestone: `M4`
- targetModule: `Quantum`
- language / stack: `Python / service helper library`
- repoType: `snapshot`
- repo local path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\wrapper-safe-python`
- repo remote provider: `github`
- repo remote URL: `https://github.com/ARQ-Sec/wrapper-safe-python`
- default branch: `main`
- scan modes intended for this scenario: `HEAD_SNAPSHOT`
- branch scopes intended for this scenario: `SINGLE_BRANCH`
- project-local dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\wrapper-safe-python\validation\generated-project-dossier.md`
- required alias dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\Q-V4-PY-004\validation\generated-project-dossier.md`
- central dossier report path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\reports\generated-project-dossiers\Q-V4-PY-004.md`

## 2. Business Context and Why This Repo Is Realistic

This scenario models `Python utility repo with scary naming, import-only crypto references, and one real weak RNG path.` as a `Python / service helper library` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Quantum precision for Python wrapper-safe surfaces versus actionable weak RNG calls.`.

## 3. Architecture Summary

- Major components: `app, docs, ops, scripts, sql, tests, validation`
- Runtime role: `Python utility repo with scary naming, import-only crypto references, and one real weak RNG path.`
...
- Crypto/TLS flow if relevant: `legacy_nonce.py`
...
| app/security/legacy_nonce.py | live-code | 5 | Runtime business module contributing to Legacy Nonce. | yes | yes | no | yes | yes | no |
| app/security/scary_random_wrapper.py | live-code | 4 | Runtime business module contributing to Scary Random Wrapper. | no | yes | no | yes | yes | no |
...
- Path: `app/security/legacy_nonce.py`
  Why it should be detected: scenario declares `weak-reset-token` as a live positive surface.
  Target module: `Quantum`
  Finding family / rule family expectation: `random`
  Head/history behavior: `head-only`
  Explainability expectation: resolvedValue~random, queryFamily~python
...
- `app/security/legacy_nonce.py`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
- `app/security/scary_random_wrapper.py`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
...
- Command: `C:\Python313\python.EXE -m compileall app tests scripts`
  ...
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\wrapper-safe-python\validation\runnability-logs\build-01.log`
...
- Command: `C:\Python313\python.EXE -m pytest -q`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\wrapper-safe-python\validation\runnability-logs\test-01.log`
...
- Command: `C:\Python313\python.EXE scripts/smoke.py`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\wrapper-safe-python\validation\runnability-logs\smoke-01.log`
...
```text
0001: # wrapper-safe-python
0002: 
0003: Python utility repo with scary naming, import-only crypto references, and one real weak RNG path.
0004: 
...
### `app/security/hash_inventory.py`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business module contributing to Hash Inventory. It is executable/live in the assembled repository.
...
0004:     return ('sha256', 'sha512')
```

### `app/security/legacy_nonce.py`

- Why this file matters: `live-code` file with expectation `must_find`.
- Detailed summary: Runtime business module contributing to Legacy Nonce. It is executable/live in the assembled repository.
...
0001: import random
0002: 
0003: def issue_nonce() -> str:
0004:     generator = random.Random()
0005:     return f"nonce-{generator.randint(100000, 999999)}"
```

### `app/security/scary_random_wrapper.py`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business module contributing to Scary Random Wrapper. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:
...
0001: import secrets
0002: 
0003: def random_or_not(value: bytes) -> str:
0004:     return secrets.token_hex(16)
```
...
- Total LOC considered for authored/generated project content: `5035`
- Synthetic filler / inflation LOC: `4730`
- Synthetic filler ratio: `93.94%`

| category | LOC |
| --- | ---: |
| live business code | 98 |
| live config | 0 |
| tests | 32 |
| docs | 11 |
| scripts | 1 |
| fixtures | 0 |
| vendor/generated | 148 |
...
- path: `legacy_nonce.py`
  module: `Quantum`
  expected rule/finding family: `random`
  expected branch/ref behavior: `head-only`
  expected explainability behavior: resolvedValue~random, queryFamily~python

### must_not_find

- path: `scary_random_wrapper.py`
...
- path: `hash_inventory.py`
  why finding must not appear: scenario declares this path clean despite adversarial content or naming.
  negative type: `near-real negative`
- path: `docs/crypto-guidance.md`
  why finding must not appear: scenario declares this path clean despite adversarial content or naming.
  negative type: `near-real negative`
...
- `legacy_nonce.py`: resolvedValue~`random`, queryFamily~`python`
