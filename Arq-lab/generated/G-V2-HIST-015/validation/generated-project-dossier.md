# Generated Project Dossier - G-V2-HIST-015

## 1. Scenario Identity

- scenarioId: `G-V2-HIST-015`
- scenarioName: Guardian all-branches behavior when main is cert-only but a feature branch carries a private key.
- milestone: `M2`
- targetModule: `Guardian`
- language / stack: `Python + PKI ops`
- repoType: `mixed`
- repo local path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\branch-private-key-divergence`
- repo remote provider: `github`
- repo remote URL: `https://github.com/ARQ-Sec/branch-private-key-divergence`
- default branch: `main`
...
- project-local dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\branch-private-key-divergence\validation\generated-project-dossier.md`
- required alias dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\G-V2-HIST-015\validation\generated-project-dossier.md`
- central dossier report path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\reports\generated-project-dossiers\G-V2-HIST-015.md`

## 2. Business Context and Why This Repo Is Realistic

This scenario models `Feature branch carries a private key while main stays on public cert and CSR material only.` as a `Python + PKI ops` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Guardian all-branches behavior when main is cert-only but a feature branch carries a private key.`.

## 3. Architecture Summary

- Major components: `app, docs, ops, scripts, sql, tests, validation`
- Runtime role: `Feature branch carries a private key while main stays on public cert and CSR material only.`
- Config flow: `No dedicated live-config files; runtime behavior is code-driven.`
- Secret flow: `feature-private.pem`
...
| validation/generated-project-dossier.md | generated | 683 | Machine-readable validation contract or generated audit artifact for this scenario. | no | no | yes | no | no | no |
...
- Path: `feature-private.pem`
  Why it should be detected: scenario declares `feature-private-key` as a live positive surface.
...
- `feature/private-key-hotfix` tip: `43adfd26e3937f8c0937a49c6e5ae6d54d89c647`; diverges from `main` at `85943eb4ef5b45664adc1d3f0bccc5f22166c7fd`
- `main` tip: `85943eb4ef5b45664adc1d3f0bccc5f22166c7fd`
- `release/2026.06` tip: `b03bfb0c2868af3c3a5b245b2a43bcc7524affdf`; diverges from `main` at `85943eb4ef5b45664adc1d3f0bccc5f22166c7fd`

Commit order:

- `0b834e76b37bbfd69984c77ed8c80ab8af8221c0` `c001 bootstrap`: initial clean or baseline assembly.
- `85943eb4ef5b45664adc1d3f0bccc5f22166c7fd` `c002 keep main cert and csr only`: scenario state change.
- `43adfd26e3937f8c0937a49c6e5ae6d54d89c647` `c003 feature branch private key`: scenario state change.
- `b03bfb0c2868af3c3a5b245b2a43bcc7524affdf` `c004 release branch clean`: removes or neutralizes a prior signal.

Expected final head/history state:

- `feature-private-key` on `feature-private.pem` should behave as `head-only`.
...
- Command: `C:\Python313\python.EXE -m compileall app tests scripts`
  ...
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\branch-private-key-divergence\validation\runnability-logs\build-01.log`
...
- Command: `C:\Python313\python.EXE -m pytest -q`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\branch-private-key-divergence\validation\runnability-logs\test-01.log`
...
- Command: `C:\Python313\python.EXE scripts/smoke.py`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\branch-private-key-divergence\validation\runnability-logs\smoke-01.log`
...
```text
0002: 
0003: Feature branch carries a private key while main stays on public cert and CSR material only.
0004: 
```
...
- path: `docs/certs/public-chain.pem`
...
- What this scenario is proving: `Guardian all-branches behavior when main is cert-only but a feature branch carries a private key.`
