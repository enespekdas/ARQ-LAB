# Generated Project Dossier - Q-V3-CS-004

## 1. Scenario Identity

- scenarioId: `Q-V3-CS-004`
- scenarioName: Quantum precision for C# crypto inventory versus real weak-hash use.
- milestone: `M3`
- targetModule: `Quantum`
- language / stack: `C# / .NET 8`
- repoType: `snapshot`
- repo local path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-usage-csharp`
- repo remote provider: `github`
- repo remote URL: `https://github.com/ARQ-Sec/crypto-usage-csharp`
- default branch: `main`
- scan modes intended for this scenario: `HEAD_SNAPSHOT`
- branch scopes intended for this scenario: `SINGLE_BRANCH`
- project-local dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-usage-csharp\validation\generated-project-dossier.md`
- required alias dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\Q-V3-CS-004\validation\generated-project-dossier.md`
- central dossier report path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\reports\generated-project-dossiers\Q-V3-CS-004.md`

## 2. Business Context and Why This Repo Is Realistic

This scenario models `C# partner SDK with one actionable weak hash and nearby import-only crypto residue.` as a `C# / .NET 8` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Quantum precision for C# crypto inventory versus real weak-hash use.`.

## 3. Architecture Summary

- Major components: `docs, ops, sql, src, tests, validation`
- Runtime role: `C# partner SDK with one actionable weak hash and nearby import-only crypto residue.`
...
- Crypto/TLS flow if relevant: `LegacyDigestService.cs`
...
|       |   `-- PartnersRegistry.cs
|       |-- Security
|       |   |-- DigestInventoryOnly.cs
|       |   |-- LegacyDigestService.cs
|       |   `-- SecureDigestFacade.cs
|       |-- Tokens
|       |   `-- TokensRegistry.cs
|       `-- Library.csproj
|-- tests
...
| docs/csharp-crypto-notes.md | docs | 1 | Documentation or explanatory material for Csharp Crypto Notes. | no | no | yes | no | no | no |
...
| src/Library/Security/DigestInventoryOnly.cs | live-code | 1 | Runtime business module contributing to Digest Inventory Only. | no | yes | no | yes | no | no |
| src/Library/Security/LegacyDigestService.cs | live-code | 1 | Runtime business service implementing Legacy Digest Service logic. | yes | yes | no | yes | no | no |
| src/Library/Security/SecureDigestFacade.cs | live-code | 1 | Runtime business module contributing to Secure Digest Facade. | no | yes | no | yes | no | no |
...
- `src/Library/Security/DigestInventoryOnly.cs`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
- `src/Library/Security/LegacyDigestService.cs`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
- `src/Library/Security/SecureDigestFacade.cs`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
...
| docs/csharp-crypto-notes.md | docs | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
...
- Command: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\.toolchains\dotnet\dotnet.exe restore src/AppHost/AppHost.csproj`
  ...
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-usage-csharp\validation\runnability-logs\build-01.log`
- Command: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\.toolchains\dotnet\dotnet.exe build src/AppHost/AppHost.csproj -c Release`
  Expected result: `build` step completes successfully.
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-usage-csharp\validation\runnability-logs\build-02.log`
...
- Command: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\.toolchains\dotnet\dotnet.exe test tests/AppHost.Tests/AppHost.Tests.csproj -c Release --no-build`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-usage-csharp\validation\runnability-logs\test-01.log`
...
- Command: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\.toolchains\dotnet\dotnet.exe run --project src/AppHost/AppHost.csproj -- --smoke`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-usage-csharp\validation\runnability-logs\smoke-01.log`
...
```text
0001: # crypto-usage-csharp
0002: 
0003: C# partner SDK with one actionable weak hash and nearby import-only crypto residue.
0004: 
...
### `docs/csharp-crypto-notes.md`

- Why this file matters: `docs` file with expectation `must_not_find`.
- Detailed summary: Documentation or explanatory material for Csharp Crypto Notes. It is non-live or protected in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`False`; protected=`True`.
- Representative excerpt:

```text
0001: Inventory-only C# notes should stay descriptive and safe.
```

### `src/Library/Security/DigestInventoryOnly.cs`
...
```text
0001: using System.Security.Cryptography; namespace Arq.Lab.Library.Security; public static class DigestInventoryOnly { public static string Supported() => SHA256.HashData(System.Text.Encoding.UTF8.GetBytes("seed")).Length.ToString(); }
```

### `src/Library/Security/LegacyDigestService.cs`
...
```text
0001: using System.Security.Cryptography; namespace Arq.Lab.Library.Security; public static class LegacyDigestService { public static byte[] Md5(byte[] value) => MD5.Create().ComputeHash(value); }
```

### `src/Library/Security/SecureDigestFacade.cs`
...
```text
0001: using System.Security.Cryptography; namespace Arq.Lab.Library.Security; public static class SecureDigestFacade { public static byte[] Sha256(byte[] value) => SHA256.HashData(value); }
```
...
- path: `SecureDigestFacade.cs`
  why finding must not appear: scenario declares this path clean despite adversarial content or naming.
  negative type: `near-real negative`
- path: `docs/csharp-crypto-notes.md`
  why finding must not appear: scenario declares this path clean despite adversarial content or naming.
