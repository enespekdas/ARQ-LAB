# Generated Project Dossier - Q-V4-TS-005

## 1. Scenario Identity

- scenarioId: `Q-V4-TS-005`
- scenarioName: Quantum should preserve semantic precision through TypeScript alias and return-based weak-hash flows.
- milestone: `M4`
- targetModule: `Quantum`
- language / stack: `TypeScript / Node`
- repoType: `snapshot`
- repo local path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-builder-node`
- repo remote provider: `github`
- repo remote URL: `https://github.com/ARQ-Sec/crypto-builder-node`
- default branch: `main`
- scan modes intended for this scenario: `HEAD_SNAPSHOT`
- branch scopes intended for this scenario: `SINGLE_BRANCH`
- project-local dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-builder-node\validation\generated-project-dossier.md`
- required alias dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\Q-V4-TS-005\validation\generated-project-dossier.md`
- central dossier report path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\reports\generated-project-dossiers\Q-V4-TS-005.md`

## 2. Business Context and Why This Repo Is Realistic

This scenario models `Node helper repo using alias and return-based wrapper chains around crypto.createHash.` as a `TypeScript / Node` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Quantum should preserve semantic precision through TypeScript alias and return-based weak-hash flows.`.

## 3. Architecture Summary

- Major components: `__tests__, dist, docs, ops, scripts, sql, src, validation`
- Runtime role: `Node helper repo using alias and return-based wrapper chains around crypto.createHash.`
...
- Crypto/TLS flow if relevant: `hashFactory.ts`
...
| dist/src/modules/security/hashFactory.js | generated | 6 | Generated or derived project artifact related to Hash Factory. | no | no | yes | yes | no | no |
| dist/src/modules/security/secureHashFactory.js | generated | 6 | Generated or derived project artifact related to Secure Hash Factory. | no | no | yes | yes | no | no |
...
| src/modules/security/hashFactory.ts | live-code | 3 | Runtime business module contributing to Hash Factory. | yes | no | no | yes | yes | no |
| src/modules/security/secureHashFactory.ts | live-code | 3 | Runtime business module contributing to Secure Hash Factory. | no | yes | no | yes | yes | no |
...
| validation/generated-project-dossier.md | generated | 1036 | Machine-readable validation contract or generated audit artifact for this scenario. | no | no | yes | no | no | no |
...
- Path: `src/modules/security/hashFactory.ts`
  Why it should be detected: scenario declares `wrapped-md5` as a live positive surface.
...
- `src/modules/security/secureHashFactory.ts`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
...
| dist/src/modules/security/hashFactory.js | generated | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
| dist/src/modules/security/secureHashFactory.js | generated | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
...
- Command: `C:\Program Files\nodejs\npm.cmd install --silent`
  ...
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-builder-node\validation\runnability-logs\build-01.log`
- Command: `C:\Program Files\nodejs\npm.cmd run build`
  Expected result: `build` step completes successfully.
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-builder-node\validation\runnability-logs\build-02.log`
...
- Command: `C:\Program Files\nodejs\npm.cmd test -- --runInBand`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-builder-node\validation\runnability-logs\test-01.log`
...
- Command: `C:\Program Files\nodejs\node.exe dist/scripts/smoke.js`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-builder-node\validation\runnability-logs\smoke-01.log`
...
```text
0001: # crypto-builder-node
0002: 
0003: Node helper repo using alias and return-based wrapper chains around crypto.createHash.
0004: 
...
### `src/modules/security/hashFactory.ts`

- Why this file matters: `live-code` file with expectation `must_find`.
- Detailed summary: Runtime business module contributing to Hash Factory. It is executable/live in the assembled repository.
- Key constructs: positive surface; near-real=`False`; protected=`False`.
- Representative excerpt:

```text
0001: import { createHash, Hash } from 'crypto';
0002: function buildLegacy(): Hash { return createHash('md5'); }
0003: export function digest(value: string): string { return buildLegacy().update(value).digest('hex'); }
```

### `src/modules/security/secureHashFactory.ts`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business module contributing to Secure Hash Factory. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: import { createHash, Hash } from 'crypto';
0002: function buildSecure(): Hash { return createHash('sha256'); }
0003: export function digest(value: string): string { return buildSecure().update(value).digest('hex'); }
```
...
| category | LOC |
| --- | ---: |
| live business code | 27 |
| live config | 0 |
| tests | 16 |
| docs | 11 |
| scripts | 1 |
| fixtures | 0 |
| vendor/generated | 240 |
...
- `hashFactory.ts`: resolvedValue~`MD5`, queryFamily~`typescript`
