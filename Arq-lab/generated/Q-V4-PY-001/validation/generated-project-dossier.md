# Generated Project Dossier - Q-V4-PY-001

## 1. Scenario Identity

- scenarioId: `Q-V4-PY-001`
- scenarioName: Quantum weak crypto patterns in Python signing helpers.
- milestone: `M4`
...
- repo remote URL in Gitea: `http://localhost:3001/arq/notification-signer-python-20260330t074019z`
- default branch: `main`
- scan modes intended for this scenario: `HEAD_SNAPSHOT`
- branch scopes intended for this scenario: `SINGLE_BRANCH`
- project-local dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\M4\notification-signer-python\validation\generated-project-dossier.md`
- required alias dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\Q-V4-PY-001\validation\generated-project-dossier.md`
- central dossier report path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\reports\generated-project-dossiers\Q-V4-PY-001.md`

## 2. Business Context and Why This Repo Is Realistic

This scenario models `Webhook signing and outbound notification auth helper.` as a `Python / FastAPI` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Quantum weak crypto patterns in Python signing helpers.`.

## 3. Architecture Summary

- Major components: `app, docs, ops, scripts, sql, tests, validation`
- Runtime role: `Webhook signing and outbound notification auth helper.`
...
- Crypto/TLS flow if relevant: `legacy_signature.py, token_factory.py`
...
| app/security/secure_signature.py | live-code | 4 | Runtime business module contributing to Secure Signature. | no | yes | no | yes | yes | no |
| app/security/token_factory.py | live-code | 5 | Runtime business module contributing to Token Factory. | yes | yes | no | yes | yes | no |
...
| validation/generated-project-dossier.md | generated | 771 | Machine-readable validation contract or generated audit artifact for this scenario. | no | no | yes | no | no | no |
...
- Path: `app/security/legacy_signature.py`
...
- Path: `app/security/token_factory.py`
  Why it should be detected: scenario declares `weak-token` as a live positive surface.
  Target module: `Quantum`
  Finding family / rule family expectation: `random`
  Head/history behavior: `head-only`
  Explainability expectation: resolvedValue~random
...
- `app/security/secure_signature.py`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
- `app/security/token_factory.py`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
...
- Command: `python -m compileall app tests scripts`
  ...
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\M4\notification-signer-python\validation\runnability-logs\build-01.log`
...
- Command: `python -m pytest -q`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\M4\notification-signer-python\validation\runnability-logs\test-01.log`
...
- Command: `python scripts/smoke.py`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\M4\notification-signer-python\validation\runnability-logs\smoke-01.log`
...
```text
0001: # notification-signer-python
0002: 
0003: Webhook signing and outbound notification auth helper.
0004: 
...
### `app/security/legacy_signature.py`
...
0004:     return hashlib.md5(payload).hexdigest()
```

### `app/security/secure_signature.py`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business module contributing to Secure Signature. It is executable/live in the assembled repository.
...
### `app/security/token_factory.py`

- Why this file matters: `live-code` file with expectation `must_find`.
- Detailed summary: Runtime business module contributing to Token Factory. It is executable/live in the assembled repository.
- Key constructs: positive surface; near-real=`True`; protected=`False`.
- Representative excerpt:
...
0001: import random
0002: 
0003: def issue_code() -> str:
0004:     generator = random.Random()
0005:     return f"{generator.randint(100000, 999999)}"
```
...
- Total LOC considered for authored/generated project content: `8430`
- Synthetic filler / inflation LOC: `4730`
- Synthetic filler ratio: `56.11%`
...
- path: `token_factory.py`
  module: `Quantum`
  expected rule/finding family: `random`
  expected branch/ref behavior: `head-only`
  expected explainability behavior: resolvedValue~random

### must_not_find

- path: `secure_signature.py`
...
- `legacy_signature.py`: resolvedValue~`MD5`, queryFamily~`python`
- `token_factory.py`: resolvedValue~`random`
...
- What this scenario is proving: `Quantum weak crypto patterns in Python signing helpers.`
- What it is not proving: comprehensive customer architecture fidelity beyond the declared validation contract.
- First outputs to inspect on failure: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\M4\notification-signer-python\validation\generated-project-dossier.md`, `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\M4\notification-signer-python\validation\generated-file-manifest.json`, comparison artifacts under the latest `runs/` directory, and the persisted runnability logs referenced above.
