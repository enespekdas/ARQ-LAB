# Generated Project Dossier - Q-V3-CS-005

## 1. Scenario Identity

- scenarioId: `Q-V3-CS-005`
- scenarioName: Quantum should separate C# inventory residue from real actionable MD5/SHA1 use under file-scoped namespaces.
- milestone: `M3`
- targetModule: `Quantum`
- language / stack: `C# / .NET 8`
- repoType: `snapshot`
- repo local path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\inventory-runtime-csharp`
- repo remote provider: `github`
- repo remote URL: `https://github.com/ARQ-Sec/inventory-runtime-csharp`
- default branch: `main`
- scan modes intended for this scenario: `HEAD_SNAPSHOT`
- branch scopes intended for this scenario: `SINGLE_BRANCH`
- project-local dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\inventory-runtime-csharp\validation\generated-project-dossier.md`
- required alias dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\Q-V3-CS-005\validation\generated-project-dossier.md`
- central dossier report path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\reports\generated-project-dossiers\Q-V3-CS-005.md`

## 2. Business Context and Why This Repo Is Realistic

This scenario models `C# crypto repo mixing inventory-only surfaces, file-scoped namespace variants, and one live weak hash path.` as a `C# / .NET 8` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Quantum should separate C# inventory residue from real actionable MD5/SHA1 use under file-scoped namespaces.`.

## 3. Architecture Summary

- Major components: `docs, ops, sql, src, tests, validation`
- Runtime role: `C# crypto repo mixing inventory-only surfaces, file-scoped namespace variants, and one live weak hash path.`
...
- Crypto/TLS flow if relevant: `LegacyDigestFactory.cs`
...
|       |   `-- PartnersRegistry.cs
|       |-- Security
|       |   |-- DigestInventoryCatalog.cs
|       |   |-- LegacyDigestFactory.cs
|       |   `-- SecureRandomFacade.cs
|       |-- Tokens
|       |   `-- TokensRegistry.cs
|       `-- Library.csproj
|-- tests
...
| src/Library/Security/DigestInventoryCatalog.cs | live-code | 1 | Runtime business module contributing to Digest Inventory Catalog. | no | yes | no | yes | no | no |
| src/Library/Security/LegacyDigestFactory.cs | live-code | 1 | Runtime business module contributing to Legacy Digest Factory. | yes | no | no | yes | no | no |
| src/Library/Security/SecureRandomFacade.cs | live-code | 1 | Runtime business module contributing to Secure Random Facade. | no | yes | no | yes | no | no |
...
- Path: `src/Library/Security/LegacyDigestFactory.cs`
  Why it should be detected: scenario declares `runtime-md5` as a live positive surface.
  Target module: `Quantum`
  Finding family / rule family expectation: `MD5`
  Head/history behavior: `head-only`
  Explainability expectation: resolvedValue~MD5, queryFamily~csharp; resolvedValue~SHA1, queryFamily~csharp
- Path: `src/Library/Security/LegacyDigestFactory.cs`
  Why it should be detected: scenario declares `runtime-sha1` as a live positive surface.
  Target module: `Quantum`
...
- `src/Library/Security/DigestInventoryCatalog.cs`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
- `src/Library/Security/SecureRandomFacade.cs`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
...
- Command: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\.toolchains\dotnet\dotnet.exe restore src/AppHost/AppHost.csproj`
  ...
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\inventory-runtime-csharp\validation\runnability-logs\build-01.log`
- Command: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\.toolchains\dotnet\dotnet.exe build src/AppHost/AppHost.csproj -c Release`
  Expected result: `build` step completes successfully.
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\inventory-runtime-csharp\validation\runnability-logs\build-02.log`
...
- Command: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\.toolchains\dotnet\dotnet.exe test tests/AppHost.Tests/AppHost.Tests.csproj -c Release --no-build`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\inventory-runtime-csharp\validation\runnability-logs\test-01.log`
...
- Command: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\.toolchains\dotnet\dotnet.exe run --project src/AppHost/AppHost.csproj -- --smoke`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\inventory-runtime-csharp\validation\runnability-logs\smoke-01.log`
...
```text
0001: # inventory-runtime-csharp
0002: 
0003: C# crypto repo mixing inventory-only surfaces, file-scoped namespace variants, and one live weak hash path.
0004: 
...
### `src/Library/Security/DigestInventoryCatalog.cs`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business module contributing to Digest Inventory Catalog. It is executable/live in the assembled repository.
...
```text
0001: using System.Security.Cryptography; namespace Arq.Lab.Library.Security; public static class LegacyDigestFactory { public static byte[] Md5(byte[] value) => MD5.Create().ComputeHash(value); public static byte[] Sha1(byte[] value) => SHA1.Create().ComputeHash(value); }
```

### `src/Library/Security/SecureRandomFacade.cs`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business module contributing to Secure Random Facade. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: using System.Security.Cryptography; namespace Arq.Lab.Library.Security; public static class SecureRandomFacade { public static int Next() => RandomNumberGenerator.GetInt32(100, 999); }
```

## 12. Line Composition and Filler Disclosure

- Total LOC considered for authored/generated project content: `5020`
- Synthetic filler / inflation LOC: `4840`
- Synthetic filler ratio: `96.41%`
...
- path: `LegacyDigestFactory.cs`
  module: `Quantum`
  expected rule/finding family: `MD5`
  expected branch/ref behavior: `head-only`
  expected explainability behavior: resolvedValue~MD5, queryFamily~csharp; resolvedValue~SHA1, queryFamily~csharp
- path: `LegacyDigestFactory.cs`
  module: `Quantum`
...
- path: `DigestInventoryCatalog.cs`
...
- `LegacyDigestFactory.cs`: resolvedValue~`MD5`, queryFamily~`csharp`
- `LegacyDigestFactory.cs`: resolvedValue~`SHA1`, queryFamily~`csharp`
