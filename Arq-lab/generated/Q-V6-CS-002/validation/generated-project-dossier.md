# Generated Project Dossier - Q-V6-CS-002

## 1. Scenario Identity

- scenarioId: `Q-V6-CS-002`
- scenarioName: Quantum trust bypass patterns in C# HttpClient registration.
- milestone: `M6`
...
- repo remote URL in Gitea: `http://localhost:3001/arq/admin-integrations-csharp-20260330t173443z`
- default branch: `main`
...
This scenario models `Admin integration service consuming partner APIs.` as a `C# / .NET 8` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Quantum trust bypass patterns in C# HttpClient registration.`.

## 3. Architecture Summary

- Major components: `docs, ops, sql, src, tests, validation`
- Runtime role: `Admin integration service consuming partner APIs.`
...
- Crypto/TLS flow if relevant: `InsecurePartnerClientRegistration.cs`
...
|       |-- Http
|       |   |-- InsecurePartnerClientRegistration.cs
|       |   `-- SecurePartnerClientRegistration.cs
|       |-- Invoices
|       |   `-- InvoicesRegistry.cs
|       |-- Partners
|       |   `-- PartnersRegistry.cs
...
| src/Library/Http/InsecurePartnerClientRegistration.cs | live-code | 1 | Outbound integration client for Insecure Partner Client Registration behavior. | yes | yes | no | yes | no | no |
| src/Library/Http/SecurePartnerClientRegistration.cs | live-code | 1 | Outbound integration client for Secure Partner Client Registration behavior. | no | yes | no | yes | no | no |
...
- Path: `src/Library/Http/InsecurePartnerClientRegistration.cs`
  Why it should be detected: scenario declares `accept-any-cert` as a live positive surface.
...
- `src/Library/Http/InsecurePartnerClientRegistration.cs`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
- `src/Library/Http/SecurePartnerClientRegistration.cs`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
...
```text
0001: # admin-integrations-csharp
0002: 
0003: Admin integration service consuming partner APIs.
0004: 
...
### `src/Library/Http/InsecurePartnerClientRegistration.cs`

- Why this file matters: `live-code` file with expectation `must_find`.
- Detailed summary: Outbound integration client for Insecure Partner Client Registration behavior. It is executable/live in the assembled repository.
- Key constructs: positive surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: using System.Net.Http; namespace Arq.Lab.Library.Http; public static class InsecurePartnerClientRegistration { public static HttpClientHandler Build() => new HttpClientHandler { ServerCertificateCustomValidationCallback = HttpClientHandler.DangerousAcceptAnyServerCertificateValidator }; }
```

### `src/Library/Http/SecurePartnerClientRegistration.cs`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Outbound integration client for Secure Partner Client Registration behavior. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: using System.Net.Http; namespace Arq.Lab.Library.Http; public static class SecurePartnerClientRegistration { public static HttpClientHandler Build() => new HttpClientHandler(); }
```
...
- path: `InsecurePartnerClientRegistration.cs`
  module: `Quantum`
...
- path: `SecurePartnerClientRegistration.cs`
...
- path: `docs/httpclient-registration.md`
  why finding must not appear: scenario declares this path clean despite adversarial content or naming.
  negative type: `near-real negative`
...
- `InsecurePartnerClientRegistration.cs`: resolvedValue~`DangerousAcceptAnyServerCertificateValidator`, queryFamily~`csharp`
