# Generated Project Dossier - G-V1-CONFIG-004

## 1. Scenario Identity

- scenarioId: `G-V1-CONFIG-004`
- scenarioName: Guardian live secret surfaces in a config-heavy operations repository.
- milestone: `M1`
- targetModule: `Guardian`
- language / stack: `Config-heavy repo`
- repoType: `snapshot`
- repo local path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\legacy-admin-config`
- repo remote provider: `github`
- repo remote URL: `https://github.com/ARQ-Sec/legacy-admin-config`
- default branch: `main`
- scan modes intended for this scenario: `HEAD_SNAPSHOT`
- branch scopes intended for this scenario: `SINGLE_BRANCH`
- project-local dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\legacy-admin-config\validation\generated-project-dossier.md`
- required alias dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\G-V1-CONFIG-004\validation\generated-project-dossier.md`
- central dossier report path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\reports\generated-project-dossiers\G-V1-CONFIG-004.md`

## 2. Business Context and Why This Repo Is Realistic

This scenario models `Legacy admin portal and provisioning configuration.` as a `Config-heavy repo` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Guardian live secret surfaces in a config-heavy operations repository.`.

## 3. Architecture Summary

- Major components: `config, deploy, docs, ops, scripts, sql, terraform, tests, validation`
- Runtime role: `Legacy admin portal and provisioning configuration.`
- Config flow: `config/keys/admin_portal.pem, config/runtime/admin-prod.yaml, config/runtime/baseline.yaml, deploy/prod/admin.properties, deploy/prod/service.properties, ops/runtime/portal.env, terraform/sample.tfvars`
- Secret flow: `config/keys/admin_portal.pem, config/runtime/admin-prod.yaml, deploy/prod/admin.properties, ops/runtime/portal.env`
- Crypto/TLS flow if relevant: `Not a Quantum-positive scenario.`
- Test surfaces: `tests/test_validation.py`
...
| config/keys/admin_portal.pem | live-config | 10 | Runtime configuration carrying environment or deployment settings for Admin Portal. | yes | no | no | yes | yes | no |
| config/runtime/admin-prod.yaml | live-config | 2 | Runtime configuration carrying environment or deployment settings for Admin Prod. | yes | no | no | yes | yes | no |
...
| deploy/prod/admin.properties | live-config | 1 | Runtime configuration carrying environment or deployment settings for Admin. | yes | no | no | yes | yes | no |
...
| ops/runtime/portal.env | live-config | 1 | Runtime configuration carrying environment or deployment settings for Portal. | yes | no | no | yes | yes | no |
...
| terraform/sample.tfvars | live-config | 1 | Runtime configuration carrying environment or deployment settings for Sample. | no | yes | no | yes | yes | no |
...
| validation/generated-file-manifest.json | generated | 2200 | Machine-readable validation contract or generated audit artifact for this scenario. | no | no | yes | no | no | no |
| validation/generated-project-dossier.md | generated | 723 | Machine-readable validation contract or generated audit artifact for this scenario. | no | no | yes | no | no | no |
| validation/generated-tree.txt | generated | 175 | Machine-readable validation contract or generated audit artifact for this scenario. | no | no | yes | no | no | no |
...
- Path: `config/runtime/admin-prod.yaml`
  Why it should be detected: scenario declares `ops-runtime` as a live positive surface.
...
- Path: `ops/runtime/portal.env`
  Why it should be detected: scenario declares `service-account` as a live positive surface.
  Target module: `Guardian`
  Finding family / rule family expectation: `guardian.generic-api-key`
  Head/history behavior: `head-only`
  Explainability expectation: No strict scenario-specific explainability contract beyond normal detail capture.
- Path: `config/keys/admin_portal.pem`
  ...
  Head/history behavior: `head-only`
  Explainability expectation: No strict scenario-specific explainability contract beyond normal detail capture.
- Path: `deploy/prod/admin.properties`
  Why it should be detected: scenario declares `active-properties` as a live positive surface.
  Target module: `Guardian`
  Finding family / rule family expectation: `guardian.generic-api-key`
  Head/history behavior: `head-only`
  Explainability expectation: No strict scenario-specific explainability contract beyond normal detail capture.
...
- `terraform/sample.tfvars`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
...
- Command: `C:\Python313\python.EXE scripts/validate_repo.py`
  ...
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\legacy-admin-config\validation\runnability-logs\build-01.log`
...
- Command: `C:\Python313\python.EXE -m pytest -q`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\legacy-admin-config\validation\runnability-logs\test-01.log`
...
- Command: `C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe -ExecutionPolicy Bypass -File scripts/smoke.ps1`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\legacy-admin-config\validation\runnability-logs\smoke-01.log`
...
```text
0001: # legacy-admin-config
0002: 
0003: Legacy admin portal and provisioning configuration.
0004: 
...
### `config/keys/admin_portal.pem`

- Why this file matters: `live-config` file with expectation `must_find`.
- Detailed summary: Runtime configuration carrying environment or deployment settings for Admin Portal. It is executable/live in the assembled repository.
...
### `config/runtime/admin-prod.yaml`

- Why this file matters: `live-config` file with expectation `must_find`.
- Detailed summary: Runtime configuration carrying environment or deployment settings for Admin Prod. It is executable/live in the assembled repository.
- Key constructs: positive surface; near-real=`False`; protected=`False`.
- Representative excerpt:

```text
0001: auth:
0002:   secret: 0123456789ABCDEFA31a
```

### `deploy/prod/admin.properties`

- Why this file matters: `live-config` file with expectation `must_find`.
- Detailed summary: Runtime configuration carrying environment or deployment settings for Admin. It is executable/live in the assembled repository.
- Key constructs: positive surface; near-real=`False`; protected=`False`.
- Representative excerpt:

```text
0001: admin.password=Ab9K2mQ7pL4xR8nT5vW1Yz7Ck3Hs6Fq2
```

### `ops/runtime/portal.env`

- Why this file matters: `live-config` file with expectation `must_find`.
- Detailed summary: Runtime configuration carrying environment or deployment settings for Portal. It is executable/live in the assembled repository.
- Key constructs: positive surface; near-real=`False`; protected=`False`.
- Representative excerpt:

```text
0001: PORTAL_TOKEN=R6mV2qL9xC4tY8nP1kD5hF7sW3zB0j
```
...
- Detailed summary: Runtime configuration carrying environment or deployment settings for Sample. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: admin_password = "example"
```
