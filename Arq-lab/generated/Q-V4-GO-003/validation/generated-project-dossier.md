# Generated Project Dossier - Q-V4-GO-003

## 1. Scenario Identity

- scenarioId: `Q-V4-GO-003`
- scenarioName: Quantum should catch real Go md5/sha1 runtime use without treating docs and tests as live signals.
- milestone: `M4`
- targetModule: `Quantum`
- language / stack: `Go / service runtime`
- repoType: `snapshot`
- repo local path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-runtime-go`
- repo remote provider: `github`
- repo remote URL: `https://github.com/ARQ-Sec/crypto-runtime-go`
- default branch: `main`
- scan modes intended for this scenario: `HEAD_SNAPSHOT`
- branch scopes intended for this scenario: `SINGLE_BRANCH`
- project-local dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-runtime-go\validation\generated-project-dossier.md`
- required alias dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\Q-V4-GO-003\validation\generated-project-dossier.md`
- central dossier report path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\reports\generated-project-dossiers\Q-V4-GO-003.md`

## 2. Business Context and Why This Repo Is Realistic

This scenario models `Go runtime repo with one real weak digest and nearby docs/test/import noise.` as a `Go / service runtime` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Quantum should catch real Go md5/sha1 runtime use without treating docs and tests as live signals.`.

## 3. Architecture Summary

- Major components: `cmd, docs, internal, ops, sql, validation`
- Runtime role: `Go runtime repo with one real weak digest and nearby docs/test/import noise.`
...
- Crypto/TLS flow if relevant: `legacy_hash.go`
...
| cmd/service/main.go | live-code | 3 | Runtime business module contributing to Main. | no | no | no | yes | yes | yes |
...
| docs/crypto-notes.md | docs | 1 | Documentation or explanatory material for Crypto Notes. | no | no | yes | no | no | no |
| go.mod | build/deploy | 3 | Build or deployment definition shaping how Go is compiled, packaged, or released. | no | no | no | yes | yes | no |
| internal/bootstrap/bootstrap.go | live-code | 5 | Runtime business module contributing to Bootstrap. | no | no | no | yes | yes | no |
| internal/bootstrap/bootstrap_test.go | live-code | 3 | Runtime business module contributing to Bootstrap Test. | no | no | no | yes | yes | no |
| internal/claims/claims.go | live-code | 5 | Runtime business module contributing to Claims. | no | no | no | yes | yes | no |
| internal/claims/claims_test.go | live-code | 3 | Runtime business module contributing to Claims Test. | no | no | no | yes | yes | no |
| internal/devices/devices.go | live-code | 5 | Runtime business module contributing to Devices. | no | no | no | yes | yes | no |
| internal/devices/devices_test.go | live-code | 3 | Runtime business module contributing to Devices Test. | no | no | no | yes | yes | no |
| internal/enrollment/enrollment.go | live-code | 5 | Runtime business module contributing to Enrollment. | no | no | no | yes | yes | no |
| internal/enrollment/enrollment_test.go | live-code | 3 | Runtime business module contributing to Enrollment Test. | no | no | no | yes | yes | no |
| internal/manifests/manifests.go | live-code | 5 | Runtime business module contributing to Manifests. | no | no | no | yes | yes | no |
...
| internal/security/hash_inventory_test.go | live-code | 3 | Runtime business module contributing to Hash Inventory Test. | no | yes | no | yes | yes | no |
...
- Path: `internal/security/legacy_hash.go`
  Why it should be detected: scenario declares `md5-runtime` as a live positive surface.
  Target module: `Quantum`
  Finding family / rule family expectation: `MD5`
  Head/history behavior: `head-only`
  Explainability expectation: resolvedValue~MD5; resolvedValue~SHA1
- Path: `internal/security/legacy_hash.go`
  Why it should be detected: scenario declares `sha1-runtime` as a live positive surface.
  Target module: `Quantum`
...
- `internal/security/hash_inventory_test.go`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
...
| docs/crypto-notes.md | docs | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
...
- Command: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\.toolchains\go\bin\go.exe test ./... -run TestBuildSmoke -count=1`
  ...
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-runtime-go\validation\runnability-logs\build-01.log`
...
- Command: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\.toolchains\go\bin\go.exe test ./... -count=1`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-runtime-go\validation\runnability-logs\test-01.log`
...
- Command: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\.toolchains\go\bin\go.exe run ./cmd/service --smoke`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\crypto-runtime-go\validation\runnability-logs\smoke-01.log`
...
```text
0001: # crypto-runtime-go
0002: 
0003: Go runtime repo with one real weak digest and nearby docs/test/import noise.
0004: 
...
- Detailed summary: Documentation or explanatory material for Crypto Notes. It is non-live or protected in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`False`; protected=`True`.
- Representative excerpt:

```text
0001: md5 and sha1 examples live only in archived docs, not runtime guidance.
```
...
```text
0001: module arq.lab/go
0002: 
0003: go 1.22
```

### `internal/security/hash_inventory_test.go`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business module contributing to Hash Inventory Test. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: package security
0002: import "testing"
0003: func TestInventoryOnly(t *testing.T) { left, right := Digest([]byte("seed")); if len(left) == 0 || len(right) == 0 { t.Fatal("expected digest output") } }
```

### `internal/security/legacy_hash.go`
...
0005:     "crypto/sha1"
0006: )
0008: func Digest(input []byte) ([]byte, []byte) {
0009:     left := md5.New()
0010:     left.Write(input)
0011:     right := sha1.New()
0012:     right.Write(input)
```

## 12. Line Composition and Filler Disclosure

- Total LOC considered for authored/generated project content: `5073`
- Synthetic filler / inflation LOC: `4840`
- Synthetic filler ratio: `95.41%`

| category | LOC |
| --- | ---: |
| live business code | 68 |
| live config | 0 |
| tests | 0 |
| docs | 12 |
| scripts | 0 |
...
- path: `legacy_hash.go`
  ...
  expected explainability behavior: resolvedValue~MD5; resolvedValue~SHA1
- path: `legacy_hash.go`
  module: `Quantum`
...
- `legacy_hash.go`: resolvedValue~`MD5`
- `legacy_hash.go`: resolvedValue~`SHA1`
...
- What this scenario is proving: `Quantum should catch real Go md5/sha1 runtime use without treating docs and tests as live signals.`
