# Generated Project Dossier - Q-V5-PY-003

## 1. Scenario Identity

- scenarioId: `Q-V5-PY-003`
- scenarioName: Quantum should keep active verify=False while ignoring example, comment, and test-only surfaces.
- milestone: `M5`
- targetModule: `Quantum`
- language / stack: `Python / worker service`
- repoType: `snapshot`
- repo local path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-usage-python`
- repo remote provider: `github`
- repo remote URL: `https://github.com/ARQ-Sec/tls-usage-python`
- default branch: `main`
- scan modes intended for this scenario: `HEAD_SNAPSHOT`
- branch scopes intended for this scenario: `SINGLE_BRANCH`
- project-local dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-usage-python\validation\generated-project-dossier.md`
- required alias dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\Q-V5-PY-003\validation\generated-project-dossier.md`
- central dossier report path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\reports\generated-project-dossiers\Q-V5-PY-003.md`

## 2. Business Context and Why This Repo Is Realistic

This scenario models `Python client repo with one active verify=False path and multiple dormant examples nearby.` as a `Python / worker service` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Quantum should keep active verify=False while ignoring example, comment, and test-only surfaces.`.

## 3. Architecture Summary

- Major components: `app, docs, examples, ops, scripts, sql, tests, validation`
- Runtime role: `Python client repo with one active verify=False path and multiple dormant examples nearby.`
...
- Crypto/TLS flow if relevant: `partner_sync_client.py`
- Test surfaces: `tests/test_audit_service.py, tests/test_delivery_service.py, tests/test_dispatch_service.py, tests/test_metrics_service.py, tests/test_preferences_service.py, tests/test_retry_service.py, tests/test_routing_service.py, tests/test_templates_service.py, tests/test_tls_example.py`
...
| app/clients/partner_sync_client.py | live-code | 4 | Outbound integration client for Partner Sync Client behavior. | yes | yes | no | yes | yes | no |
...
| docs/tls-usage.md | docs | 1 | Documentation or explanatory material for Tls Usage. | no | no | yes | no | no | no |
| examples/tls_example.py | docs | 3 | Documentation or explanatory material for Tls Example. | no | no | yes | no | no | no |
...
| tests/test_tls_example.py | test | 2 | Automated test surface covering Test Tls Example behavior. | no | no | yes | yes | yes | no |
...
| validation/generated-tree.txt | generated | 193 | Machine-readable validation contract or generated audit artifact for this scenario. | no | no | yes | no | no | no |
...
- Path: `app/clients/partner_sync_client.py`
  Why it should be detected: scenario declares `verify-false-live` as a live positive surface.
  Target module: `Quantum`
  Finding family / rule family expectation: `verify=False`
  Head/history behavior: `head-only`
  Explainability expectation: resolvedValue~verify=False, queryFamily~ssl

## 7. Near-Real Negative Surfaces

- `app/clients/partner_sync_client.py`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
...
| docs/tls-usage.md | docs | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
| examples/tls_example.py | docs | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
...
| tests/test_tls_example.py | test | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
...
- Command: `C:\Python313\python.EXE -m compileall app tests scripts`
  ...
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-usage-python\validation\runnability-logs\build-01.log`
...
- Command: `C:\Python313\python.EXE -m pytest -q`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-usage-python\validation\runnability-logs\test-01.log`
...
- Command: `C:\Python313\python.EXE scripts/smoke.py`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-usage-python\validation\runnability-logs\smoke-01.log`
...
```text
0002: 
0003: Python client repo with one active verify=False path and multiple dormant examples nearby.
0004: 
```

### `app/clients/partner_sync_client.py`

- Why this file matters: `live-code` file with expectation `must_find`.
- Detailed summary: Outbound integration client for Partner Sync Client behavior. It is executable/live in the assembled repository.
...
- Detailed summary: Documentation or explanatory material for Tls Usage. It is non-live or protected in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`False`; protected=`True`.
- Representative excerpt:

```text
0001: Example snippets may mention verify=False but remain non-live.
```

### `examples/tls_example.py`

- Why this file matters: `docs` file with expectation `must_not_find`.
- Detailed summary: Documentation or explanatory material for Tls Example. It is non-live or protected in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`False`; protected=`True`.
- Representative excerpt:

```text
0002: 
0003: response = requests.get('https://example.invalid', verify=False)
```
...
### `tests/test_tls_example.py`

- Why this file matters: `test` file with expectation `must_not_find`.
- Detailed summary: Automated test surface covering Test Tls Example behavior. It is non-live or protected in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`False`; protected=`True`.
- Representative excerpt:

```text
0001: def test_example_placeholder() -> None:
0002:     assert True
```

## 12. Line Composition and Filler Disclosure

- Total LOC considered for authored/generated project content: `5037`
- Synthetic filler / inflation LOC: `4730`
- Synthetic filler ratio: `93.91%`

| category | LOC |
| --- | ---: |
| live business code | 89 |
| live config | 0 |
| tests | 34 |
| docs | 15 |
| scripts | 1 |
...
- path: `partner_sync_client.py`
  module: `Quantum`
  expected rule/finding family: `verify=False`
  expected branch/ref behavior: `head-only`
  expected explainability behavior: resolvedValue~verify=False, queryFamily~ssl

### must_not_find

- path: `examples/tls_example.py`
...
- path: `tests/test_tls_example.py`
  why finding must not appear: scenario declares this path clean despite adversarial content or naming.
  negative type: `protected negative`
- path: `docs/tls-usage.md`
  why finding must not appear: scenario declares this path clean despite adversarial content or naming.
  negative type: `protected negative`
...
- `partner_sync_client.py`: resolvedValue~`verify=False`, queryFamily~`ssl`
