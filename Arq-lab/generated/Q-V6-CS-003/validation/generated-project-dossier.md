# Generated Project Dossier - Q-V6-CS-003

## 1. Scenario Identity

- scenarioId: `Q-V6-CS-003`
- scenarioName: Quantum should keep the live DangerousAcceptAny registration while leaving import-only C# surfaces clean.
- milestone: `M6`
- targetModule: `Quantum`
- language / stack: `C# / .NET 8`
- repoType: `snapshot`
- repo local path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-usage-csharp`
- repo remote provider: `github`
- repo remote URL: `https://github.com/ARQ-Sec/tls-usage-csharp`
- default branch: `main`
- scan modes intended for this scenario: `HEAD_SNAPSHOT`
- branch scopes intended for this scenario: `SINGLE_BRANCH`
- project-local dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-usage-csharp\validation\generated-project-dossier.md`
- required alias dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\Q-V6-CS-003\validation\generated-project-dossier.md`
- central dossier report path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\reports\generated-project-dossiers\Q-V6-CS-003.md`

## 2. Business Context and Why This Repo Is Realistic

This scenario models `C# registration repo with one real DangerousAcceptAny binding and nearby import-only negatives.` as a `C# / .NET 8` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Quantum should keep the live DangerousAcceptAny registration while leaving import-only C# surfaces clean.`.

## 3. Architecture Summary

- Major components: `docs, ops, sql, src, tests, validation`
- Runtime role: `C# registration repo with one real DangerousAcceptAny binding and nearby import-only negatives.`
...
- Crypto/TLS flow if relevant: `InsecureServiceRegistration.cs`
- Test surfaces: `tests/AppHost.Tests/AppHost.Tests.csproj, tests/AppHost.Tests/RegistryTests.cs`
...
|       |   `-- BalancesRegistry.cs
|       |-- Callbacks
|       |   `-- CallbacksRegistry.cs
|       |-- Http
|       |   |-- HttpInventoryCatalog.cs
|       |   |-- InsecureServiceRegistration.cs
|       |   `-- SecureServiceRegistration.cs
|       |-- Invoices
|       |   `-- InvoicesRegistry.cs
|       |-- Partners
|       |   `-- PartnersRegistry.cs
...
| docs/http-registration.md | docs | 1 | Documentation or explanatory material for Http Registration. | no | no | yes | no | no | no |
...
| src/AppHost/AppHost.csproj | build/deploy | 1 | Build or deployment definition shaping how App Host is compiled, packaged, or released. | no | no | no | yes | no | yes |
| src/AppHost/Program.cs | live-code | 1 | Runtime business module contributing to Program. | no | no | no | yes | no | yes |
...
| src/Library/Http/HttpInventoryCatalog.cs | live-code | 1 | Runtime business module contributing to Http Inventory Catalog. | no | yes | no | yes | no | no |
| src/Library/Http/InsecureServiceRegistration.cs | live-code | 1 | Runtime business service implementing Insecure Service Registration logic. | yes | yes | no | yes | no | no |
| src/Library/Http/SecureServiceRegistration.cs | live-code | 1 | Runtime business service implementing Secure Service Registration logic. | no | yes | no | yes | no | no |
...
| tests/AppHost.Tests/RegistryTests.cs | test | 1 | Automated test surface covering Registry Tests behavior. | no | no | yes | no | yes | no |
...
| validation/runnability-logs/build-02.log | generated | 18 | Machine-readable validation contract or generated audit artifact for this scenario. | no | no | yes | no | no | no |
...
- Path: `src/Library/Http/InsecureServiceRegistration.cs`
  Why it should be detected: scenario declares `accept-any-registration` as a live positive surface.
  Target module: `Quantum`
  Finding family / rule family expectation: `DangerousAcceptAnyServerCertificateValidator`
  Head/history behavior: `head-only`
  Explainability expectation: resolvedValue~DangerousAcceptAnyServerCertificateValidator, queryFamily~csharp

## 7. Near-Real Negative Surfaces

- `src/Library/Http/HttpInventoryCatalog.cs`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
- `src/Library/Http/InsecureServiceRegistration.cs`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
- `src/Library/Http/SecureServiceRegistration.cs`: Path is intentionally near-real but is expected to stay clean because it is placeholder, example, masked, or otherwise non-live.
...
| docs/http-registration.md | docs | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
...
| tests/AppHost.Tests/AppHost.Tests.csproj | test | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
| tests/AppHost.Tests/RegistryTests.cs | test | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
...
- Command: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\.toolchains\dotnet\dotnet.exe restore src/AppHost/AppHost.csproj`
  ...
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-usage-csharp\validation\runnability-logs\build-01.log`
- Command: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\.toolchains\dotnet\dotnet.exe build src/AppHost/AppHost.csproj -c Release`
  Expected result: `build` step completes successfully.
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-usage-csharp\validation\runnability-logs\build-02.log`
...
- Command: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\.toolchains\dotnet\dotnet.exe test tests/AppHost.Tests/AppHost.Tests.csproj -c Release --no-build`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-usage-csharp\validation\runnability-logs\test-01.log`
...
- Command: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\.toolchains\dotnet\dotnet.exe run --project src/AppHost/AppHost.csproj -- --smoke`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\tls-usage-csharp\validation\runnability-logs\smoke-01.log`
...
```text
0001: # tls-usage-csharp
0002: 
0003: C# registration repo with one real DangerousAcceptAny binding and nearby import-only negatives.
0004: 
...
### `docs/http-registration.md`

- Why this file matters: `docs` file with expectation `must_not_find`.
- Detailed summary: Documentation or explanatory material for Http Registration. It is non-live or protected in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`False`; protected=`True`.
- Representative excerpt:

```text
0001: Inventory-only C# HttpClient registration notes remain non-live.
```

### `src/Library/Http/HttpInventoryCatalog.cs`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business module contributing to Http Inventory Catalog. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: using System.Net.Http; namespace Arq.Lab.Library.Http; public static class HttpInventoryCatalog { public static string HandlerName() => typeof(HttpClientHandler).Name; }
```

### `src/Library/Http/InsecureServiceRegistration.cs`

- Why this file matters: `live-code` file with expectation `must_find`.
- Detailed summary: Runtime business service implementing Insecure Service Registration logic. It is executable/live in the assembled repository.
- Key constructs: positive surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: using System.Net.Http; namespace Arq.Lab.Library.Http; public static class InsecureServiceRegistration { public static HttpClientHandler Build() => new HttpClientHandler { ServerCertificateCustomValidationCallback = HttpClientHandler.DangerousAcceptAnyServerCertificateValidator }; }
```

### `src/Library/Http/SecureServiceRegistration.cs`

- Why this file matters: `live-code` file with expectation `must_not_find`.
- Detailed summary: Runtime business service implementing Secure Service Registration logic. It is executable/live in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: using System.Net.Http; namespace Arq.Lab.Library.Http; public static class SecureServiceRegistration { public static HttpClientHandler Build() => new HttpClientHandler(); }
```

## 12. Line Composition and Filler Disclosure

- Total LOC considered for authored/generated project content: `5006`
- Synthetic filler / inflation LOC: `4840`
- Synthetic filler ratio: `96.68%`

| category | LOC |
| --- | ---: |
| live business code | 10 |
| live config | 0 |
| tests | 2 |
| docs | 12 |
| scripts | 0 |
...
- path: `InsecureServiceRegistration.cs`
  module: `Quantum`
  expected rule/finding family: `DangerousAcceptAnyServerCertificateValidator`
  expected branch/ref behavior: `head-only`
  expected explainability behavior: resolvedValue~DangerousAcceptAnyServerCertificateValidator, queryFamily~csharp

### must_not_find

- path: `HttpInventoryCatalog.cs`
...
- path: `SecureServiceRegistration.cs`
  why finding must not appear: scenario declares this path clean despite adversarial content or naming.
  negative type: `near-real negative`
- path: `docs/http-registration.md`
  why finding must not appear: scenario declares this path clean despite adversarial content or naming.
...
- `InsecureServiceRegistration.cs`: resolvedValue~`DangerousAcceptAnyServerCertificateValidator`, queryFamily~`csharp`
