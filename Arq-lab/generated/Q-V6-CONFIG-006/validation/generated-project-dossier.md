# Generated Project Dossier - Q-V6-CONFIG-006

## 1. Scenario Identity

- scenarioId: `Q-V6-CONFIG-006`
- scenarioName: Quantum should keep live insecure config while leaving dormant examples, comments, and fixture overlays visible but non-runtime.
- milestone: `M6`
- targetModule: `Quantum`
- language / stack: `Config-heavy repo`
- repoType: `snapshot`
- repo local path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\overlay-matrix-config`
- repo remote provider: `github`
- repo remote URL: `https://github.com/ARQ-Sec/overlay-matrix-config`
- default branch: `main`
- scan modes intended for this scenario: `HEAD_SNAPSHOT`
- branch scopes intended for this scenario: `SINGLE_BRANCH`
- project-local dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\overlay-matrix-config\validation\generated-project-dossier.md`
- required alias dossier path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\generated\Q-V6-CONFIG-006\validation\generated-project-dossier.md`
- central dossier report path: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\reports\generated-project-dossiers\Q-V6-CONFIG-006.md`

## 2. Business Context and Why This Repo Is Realistic

This scenario models `Helm, Kustomize, Terraform, and live runtime config mixed with dormant examples, comments, and fixtures.` as a `Config-heavy repo` repository. The repo is shaped to look like something an enterprise team would actually maintain so that ARQ validation is exercised against realistic layout, runtime config, supporting docs, and test collateral rather than an isolated toy snippet.

A real customer could plausibly own this repository because it bundles the operational surfaces that usually travel with the business function: runtime code, deploy/config material, tests, scripts, and enough written collateral to resemble an actively maintained internal service or support repository. The scenario goal is `Quantum should keep live insecure config while leaving dormant examples, comments, and fixture overlays visible but non-runtime.`.

## 3. Architecture Summary

- Major components: `config, deploy, docs, helm, ops, runtime, scripts, sql, tests, validation`
- Runtime role: `Helm, Kustomize, Terraform, and live runtime config mixed with dormant examples, comments, and fixtures.`
- Config flow: `config/runtime/baseline.yaml, deploy/live/envoy.yaml, deploy/prod/service.properties, runtime/live.env`
- Secret flow: `No Guardian must-find secret path in this scenario.`
- Crypto/TLS flow if relevant: `deploy/live/envoy.yaml, runtime/live.env`
- Test surfaces: `tests/fixtures/runtime.env, tests/test_validation.py`
...
| deploy/live/envoy.yaml | live-config | 2 | Runtime configuration carrying environment or deployment settings for Envoy. | yes | no | no | yes | yes | no |
...
| docs/examples.md | docs | 3 | Documentation or explanatory material for Examples. | no | no | yes | no | no | no |
| helm/examples/values.yaml | build/deploy | 2 | Build or deployment definition shaping how Values is compiled, packaged, or released. | no | yes | no | yes | yes | no |
...
| runtime/live.env | live-config | 1 | Runtime configuration carrying environment or deployment settings for Live. | yes | no | no | yes | yes | no |
...
| tests/fixtures/runtime.env | test | 1 | Automated test surface covering Runtime behavior. | no | no | yes | no | yes | no |
...
| validation/generated-file-manifest.json | generated | 2214 | Machine-readable validation contract or generated audit artifact for this scenario. | no | no | yes | no | no | no |
| validation/generated-project-dossier.md | generated | 726 | Machine-readable validation contract or generated audit artifact for this scenario. | no | no | yes | no | no | no |
| validation/generated-tree.txt | generated | 178 | Machine-readable validation contract or generated audit artifact for this scenario. | no | no | yes | no | no | no |
...
- Path: `deploy/live/envoy.yaml`
  Why it should be detected: scenario declares `live-envoy-untrusted` as a live positive surface.
...
- Path: `deploy/live/envoy.yaml`
  Why it should be detected: scenario declares `live-envoy-protocol` as a live positive surface.
  Target module: `Quantum`
  Finding family / rule family expectation: `tls_minimum_protocol_version=TLSv1_0`
  Head/history behavior: `head-only`
  Explainability expectation: resolvedValue~ACCEPT_UNTRUSTED; resolvedValue~tls_minimum_protocol_version=TLSv1_0
- Path: `runtime/live.env`
  Why it should be detected: scenario declares `live-node-disable` as a live positive surface.
  Target module: `Quantum`
  Finding family / rule family expectation: `NODE_TLS_REJECT_UNAUTHORIZED=0`
  Head/history behavior: `head-only`
  Explainability expectation: resolvedValue~NODE_TLS_REJECT_UNAUTHORIZED=0
...
| docs/examples.md | docs | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
...
| tests/fixtures/runtime.env | test | Path lives in a protected negative zone and should stay clean even if the content looks suspicious. |
...
- Command: `C:\Python313\python.EXE scripts/validate_repo.py`
  ...
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\overlay-matrix-config\validation\runnability-logs\build-01.log`
...
- Command: `C:\Python313\python.EXE -m pytest -q`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\overlay-matrix-config\validation\runnability-logs\test-01.log`
...
- Command: `C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe -ExecutionPolicy Bypass -File scripts/smoke.ps1`
  ...
  Actual result: returncode=`0`; stage state=`passed`
  Log file: `C:\Users\EnesPekdas\Desktop\ARQV2\LAB\Arq-lab\repositories\overlay-matrix-config\validation\runnability-logs\smoke-01.log`
...
```text
0002: 
0003: Helm, Kustomize, Terraform, and live runtime config mixed with dormant examples, comments, and fixtures.
0004: 
```

### `deploy/live/envoy.yaml`
...
- Detailed summary: Documentation or explanatory material for Examples. It is non-live or protected in the assembled repository.
...
### `helm/examples/values.yaml`

- Why this file matters: `build/deploy` file with expectation `must_not_find`.
- Detailed summary: Build or deployment definition shaping how Values is compiled, packaged, or released. It is non-live or protected in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`True`; protected=`False`.
- Representative excerpt:

```text
0001: envoy:
0002:   trustChainVerification: ACCEPT_UNTRUSTED
```

### `runtime/live.env`

- Why this file matters: `live-config` file with expectation `must_find`.
- Detailed summary: Runtime configuration carrying environment or deployment settings for Live. It is executable/live in the assembled repository.
- Key constructs: positive surface; near-real=`False`; protected=`False`.
- Representative excerpt:
...
### `tests/fixtures/runtime.env`

- Why this file matters: `test` file with expectation `must_not_find`.
- Detailed summary: Automated test surface covering Runtime behavior. It is non-live or protected in the assembled repository.
- Key constructs: negative or realism-supporting surface; near-real=`False`; protected=`True`.
- Representative excerpt:
...
- Total LOC considered for authored/generated project content: `5032`
- Synthetic filler / inflation LOC: `4840`
- Synthetic filler ratio: `96.18%`
...
- path: `runtime/live.env`
  ...
  expected branch/ref behavior: `head-only`
  expected explainability behavior: resolvedValue~NODE_TLS_REJECT_UNAUTHORIZED=0
...
- `deploy/live/envoy.yaml`: resolvedValue~`ACCEPT_UNTRUSTED`
- `deploy/live/envoy.yaml`: resolvedValue~`tls_minimum_protocol_version=TLSv1_0`
- `runtime/live.env`: resolvedValue~`NODE_TLS_REJECT_UNAUTHORIZED=0`
