...
def scenario_specs() -> list[ScenarioSpec]:
    specs: list[ScenarioSpec] = [
        ...
        ScenarioSpec(
            id="G-V2-HIST-014",
            milestone="M2",
            repo_name="split-concat-lineage",
            module="guardian",
            family="guardian_history_node",
            stack="TypeScript / Node",
            domain="Partner bootstrap repo with split secret construction, later canonicalization, and final cleanup.",
            summary="Guardian history/ref handling when a secret evolves from split fragments into canonical form before removal.",
            line_target="5000-8000",
            scan_plans=_guardian_history(),
            build_plan=_build_plan(NODE_BUILD),
            expected_findings=[
            ...
            expected_absent=[
                ea("docs-split-secret", "docs/split-secret.md"),
                ea("example-concat", "examples/concat-demo.ts"),
            ],
            metadata={"commitPlan": ["c001", "c002", "c003", "c004", "c005", "c006"]},
        ),
        ScenarioSpec(
            id="G-V2-HIST-015",
            milestone="M2",
            repo_name="branch-private-key-divergence",
            module="guardian",
            family="guardian_history_python",
            stack="Python + PKI ops",
            domain="Feature branch carries a private key while main stays on public cert and CSR material only.",
            summary="Guardian all-branches behavior when main is cert-only but a feature branch carries a private key.",
            line_target="5000-8000",
            scan_plans=_guardian_history(include_all_branches=True),
            build_plan=_build_plan(PYTHON_BUILD),
            expected_findings=[
                ef("feature-private-key", "feature-private.pem", rule="guardian.private-key", branch="feature/private-key-hotfix"),
            ],
            expected_absent=[
                ea("main-public-chain", "docs/certs/public-chain.pem"),
                ea("main-csr", "ops/csr/service.csr"),
            ],
            metadata={"branches": ["main", "feature/private-key-hotfix", "release/2026.06"]},
        ),
        ScenarioSpec(
            id="G-V2-HIST-016",
            milestone="M2",
            repo_name="folded-yaml-lineage",
            module="guardian",
            family="guardian_history_python",
            ...
            summary="Guardian canonicalization and history handling for multiline YAML folded secret lineage.",
            line_target="5000-8000",
            scan_plans=_guardian_history(),
            build_plan=_build_plan(PYTHON_BUILD),
            expected_findings=[
            ...
            expected_absent=[
                ea("masked-docs-yaml", "docs/masked-values.yaml"),
                ea("tutorial-placeholder", "docs/tutorials/runtime-values.md"),
            ],
            metadata={"commitPlan": ["c001", "c002", "c003", "c004", "c005"]},
        ),
        ScenarioSpec(
            id="G-V2-HIST-017",
            milestone="M2",
            repo_name="move-duplicate-cherrypick-lineage",
            module="guardian",
            family="guardian_history_java",
            stack="Java / Spring Boot",
            domain="Java repo with move, duplicate, revert, and cherry-pick operations around one transient secret.",
            summary="Guardian dedup and ref-state behavior across move, duplicate, revert, and cherry-pick lineage.",
            line_target="5000-9000",
            scan_plans=_guardian_history(include_all_branches=True),
            build_plan=_build_plan(JAVA_BUILD),
            expected_findings=[
                ef(
                    "moved-duplicate-secret",
                    "PartnerTokenArchive.java",
                    rule="guardian.generic-api-key",
                    present_on_head=False,
                    present_in_history=True,
            ...
            expected_absent=[
                ea("cherry-picked-docs", "docs/cherry-pick.md"),
                ea("safe-main-state", "HistoryRefactor.java"),
            ],
            metadata={"branches": ["main", "feature/archive-duplication", "release/2026.07"]},
        ),
        ScenarioSpec(
            id="Q-V3-JAVA-007",
            milestone="M3",
            repo_name="helper-return-java",
            module="quantum",
            family="quantum_crypto_java",
            stack="Java / Spring Boot",
            domain="Java helper library with weak hash use hidden behind helper return and static factory chaining.",
            summary="Quantum should follow helper return and static factory chains to the real weak digest use in Java.",
            line_target="5000-9000",
            scan_plans=_quantum_head(),
            build_plan=_build_plan(JAVA_BUILD),
            expected_findings=[
                ef("helper-md5", "LegacyDigestFactory.java", resolved="MD5"),
            ],
            expected_absent=[
                ea("safe-helper", "SecureDigestFactory.java"),
                ea("inventory-note", "docs/helper-digest.md"),
            ],
            explainability_expectations=[
                ee("helper-md5", "LegacyDigestFactory.java", resolved="MD5", query_family="jca"),
            ],
        ),
        ScenarioSpec(
            id="Q-V3-JAVA-008",
            milestone="M3",
            repo_name="scary-wrapper-java",
            module="quantum",
            family="quantum_crypto_java",
            stack="Java / Spring Boot",
            domain="Java crypto wrapper repo with scary naming but only one genuinely weak side path.",
            summary="Quantum should isolate the real weak side path while keeping secure scary wrappers clean.",
            line_target="5000-9000",
            scan_plans=_quantum_head(),
            build_plan=_build_plan(JAVA_BUILD),
            expected_findings=[
        ...
        ScenarioSpec(
            id="Q-V3-CS-005",
            milestone="M3",
            repo_name="inventory-runtime-csharp",
            module="quantum",
            family="quantum_crypto_csharp",
            stack="C# / .NET 8",
            domain="C# crypto repo mixing inventory-only surfaces, file-scoped namespace variants, and one live weak hash path.",
            summary="Quantum should separate C# inventory residue from real actionable MD5/SHA1 use under file-scoped namespaces.",
            line_target="5000-9000",
            scan_plans=_quantum_head(),
            build_plan=_build_plan(CSHARP_BUILD),
            expected_findings=[
                ef("runtime-md5", "LegacyDigestFactory.cs", resolved="MD5"),
                ef("runtime-sha1", "LegacyDigestFactory.cs", resolved="SHA1"),
            ],
            expected_absent=[
                ea("inventory-random", "DigestInventoryCatalog.cs"),
                ea("safe-rng", "SecureRandomFacade.cs"),
            ],
            explainability_expectations=[
                ee("runtime-md5", "LegacyDigestFactory.cs", resolved="MD5", query_family="csharp"),
                ee("runtime-sha1", "LegacyDigestFactory.cs", resolved="SHA1", query_family="csharp"),
            ],
        ),
        ScenarioSpec(
            id="Q-V4-PY-006",
            milestone="M4",
            repo_name="hash-wrapper-python",
            module="quantum",
            family="quantum_crypto_python",
            stack="Python / service helper library",
            domain="Python hash helper repo using aliases and wrappers with one real live md5 path nearby import-only traps.",
            summary="Quantum should keep the live md5 path while leaving import-only and safe wrapper files clean in Python.",
            line_target="5000-8000",
            scan_plans=_quantum_head(),
            build_plan=_build_plan(PYTHON_BUILD),
            expected_findings=[
                ef("live-md5", "live_md5.py", resolved="md5"),
            ],
            expected_absent=[
                ea("alias-inventory", "hash_alias_inventory.py"),
                ea("safe-wrapper", "safe_hash_wrapper.py"),
            ],
            explainability_expectations=[
                ee("live-md5", "live_md5.py", resolved="md5", query_family="python"),
            ],
        ),
        ScenarioSpec(
            id="Q-V4-TS-006",
            milestone="M4",
            repo_name="hash-destructure-node",
            module="quantum",
            family="quantum_crypto_node",
            stack="TypeScript / Node",
            domain="Node repo using destructured crypto helpers, alias exports, and returned object chains around createHash.",
            summary="Quantum should follow destructuring and returned object chains to the live createHash misuse in TypeScript.",
            line_target="5000-8000",
            scan_plans=_quantum_head(),
            build_plan=_build_plan(NODE_BUILD),
            expected_findings=[
                ef("destructured-md5", "hashComposer.ts", resolved="MD5"),
            ],
            expected_absent=[
                ea("safe-destructure", "safeHashComposer.ts"),
                ea("hash-catalog", "hashCatalog.ts"),
            ],
            explainability_expectations=[
                ee("destructured-md5", "hashComposer.ts", resolved="MD5", query_family="typescript"),
            ],
        ),
        ScenarioSpec(
            id="Q-V4-GO-004",
            milestone="M4",
            repo_name="hash-helper-go",
            module="quantum",
            family="quantum_crypto_go",
            stack="Go / service runtime",
            domain="Go runtime repo with weak hash use hidden behind helper indirection and surrounded by docs/test/import noise.",
            summary="Quantum should keep live weak hash helper indirection while leaving nearby docs and test inventory clean in Go.",
            line_target="5000-8000",
            scan_plans=_quantum_head(),
            build_plan=_build_plan(GO_BUILD),
            expected_findings=[
                ef("helper-md5", "helper_hash.go", resolved="MD5"),
            ],
            expected_absent=[
                ea("docs-go", "docs/hash-guidance.md"),
                ea("tests-go", "helper_hash_test.go"),
            ],
            explainability_expectations=[
                ee("helper-md5", "helper_hash.go", resolved="MD5"),
            ],
        ),
        ScenarioSpec(
            id="Q-V5-JAVA-007",
            milestone="M5",
            repo_name="tls-builder-nested-java",
            module="quantum",
            family="quantum_tls_java",
            stack="Java / Spring Boot",
            domain="Java TLS repo with SSLContext and trust manager misuse hidden behind nested helper builders.",
            summary="Quantum should follow nested helper builders to SSLContext and trust-manager misuse in Java.",
            line_target="5000-9000",
            scan_plans=_quantum_head(),
            build_plan=_build_plan(JAVA_BUILD),
            expected_findings=[
                ef("nested-trust-bypass", "NestedTrustManagerBuilder.java", resolved="X509TrustManager"),
                ef("nested-hostname-bypass", "NestedHostnameBuilder.java", resolved="HostnameVerifier-return-true"),
            ],
            expected_absent=[
                ea("safe-helper-builder", "SecureTlsFactory.java"),
            ],
            explainability_expectations=[
                ee("nested-trust-bypass", "NestedTrustManagerBuilder.java", resolved="X509TrustManager", query_family="java"),
                ee("nested-hostname-bypass", "NestedHostnameBuilder.java", resolved="HostnameVerifier-return-true", query_family="java"),
            ],
        ),
        ScenarioSpec(
            id="Q-V5-JAVA-008",
            milestone="M5",
            repo_name="safe-trustall-java",
            module="quantum",
            family="quantum_tls_java",
            stack="Java / Spring Boot",
            domain="Java TLS helper repo with trustAll-style scary naming but safe runtime behavior.",
            summary="TrustAll-style scary naming should stay clean when the wrapper remains secure at runtime.",
            line_target="5000-9000",
            scan_plans=_quantum_head(),
            build_plan=_build_plan(JAVA_BUILD),
            expected_absent=[
                ea("safe-trustall-wrapper", "TrustAllStyleSafeClient.java"),
                ea("safe-hostname-wrapper", "TrustAllStyleHostnameFacade.java"),
                ea("docs-note", "docs/safe-trustall.md"),
            ],
        ),
        ScenarioSpec(
            id="Q-V5-PY-004",
            milestone="M5",
            repo_name="tls-kwargs-python",
            module="quantum",
            family="quantum_tls_python",
            stack="Python / client wrapper",
            domain="Python repo where verify=False flows through kwargs passthrough and a session wrapper.",
            summary="Quantum should surface live verify=False through kwargs passthrough and wrapper indirection in Python.",
            line_target="5000-8000",
            scan_plans=_quantum_head(),
            build_plan=_build_plan(PYTHON_BUILD),
            expected_findings=[
                ef("kwargs-verify-false", "session_wrapper.py", resolved="verify=False"),
            ],
            expected_absent=[
                ea("safe-wrapper", "safe_session_wrapper.py"),
                ea("example-only", "examples/session_example.py"),
            ],
            explainability_expectations=[
                ee("kwargs-verify-false", "session_wrapper.py", resolved="verify=False", query_family="ssl"),
            ],
        ),
        ScenarioSpec(
            id="Q-V6-TS-004",
            milestone="M6",
            repo_name="tls-compose-node",
            module="quantum",
            family="quantum_tls_node",
            stack="TypeScript / Node",
            domain="Node TLS repo composing partial configs with spread and Object.assign before returning the live agent.",
            summary="Quantum should follow rejectUnauthorized through spread, Object.assign, and partial config composition in TypeScript.",
            line_target="5000-8000",
            scan_plans=_quantum_head(),
            build_plan=_build_plan(NODE_BUILD),
            expected_findings=[
                ef("composed-reject-unauthorized", "composeTls.ts", resolved="rejectUnauthorized"),
            ],
            expected_absent=[
                ea("safe-compose", "safeComposeTls.ts"),
                ea("docs-compose", "docs/tls-compose.md"),
            ],
            explainability_expectations=[
                ee("composed-reject-unauthorized", "composeTls.ts", resolved="rejectUnauthorized", query_family="typescript"),
            ],
        ),
        ScenarioSpec(
            id="Q-V6-CS-004",
            milestone="M6",
            repo_name="tls-indirection-csharp",
            module="quantum",
            family="quantum_tls_csharp",
            stack="C# / .NET 8",
            domain="C# TLS repo carrying DangerousAcceptAny through delegate, field, and property indirection.",
            summary="Quantum should surface DangerousAcceptAny when it flows through delegate and field indirection in C#.",
            line_target="5000-9000",
            scan_plans=_quantum_head(),
            build_plan=_build_plan(CSHARP_BUILD),
            expected_findings=[
                ef("delegate-accept-any", "HttpClientPolicy.cs", resolved="DangerousAcceptAnyServerCertificateValidator"),
            ],
            expected_absent=[
                ea("inventory-http", "HttpPolicyInventory.cs"),
                ea("safe-policy", "SecureHttpClientPolicy.cs"),
            ],
            explainability_expectations=[
                ee("delegate-accept-any", "HttpClientPolicy.cs", resolved="DangerousAcceptAnyServerCertificateValidator", query_family="csharp"),
            ],
        ),
        ScenarioSpec(
            id="Q-V6-CONFIG-006",
            milestone="M6",
            repo_name="overlay-matrix-config",
            module="quantum",
            family="quantum_tls_config",
            stack="Config-heavy repo",
            domain="Helm, Kustomize, Terraform, and live runtime config mixed with dormant examples, comments, and fixtures.",
            summary="Quantum should keep live insecure config while leaving dormant examples, comments, and fixture overlays visible but non-runtime.",
            line_target="5000-8000",
            scan_plans=_quantum_head(),
            build_plan=_build_plan(CONFIG_BUILD),
            expected_findings=[
                ef("live-envoy-untrusted", "deploy/live/envoy.yaml", resolved="ACCEPT_UNTRUSTED"),
                ef("live-envoy-protocol", "deploy/live/envoy.yaml", resolved="tls_minimum_protocol_version=TLSv1_0"),
                ef("live-node-disable", "runtime/live.env", resolved="NODE_TLS_REJECT_UNAUTHORIZED=0"),
            ],
            expected_absent=[
                ea("commented-overlay", "docs/examples.md"),
                ea("fixture-values", "tests/fixtures"),
                ea("example-helm", "helm/examples"),
            ],
            explainability_expectations=[
                ee("live-envoy-untrusted", "deploy/live/envoy.yaml", resolved="ACCEPT_UNTRUSTED"),
                ee("live-envoy-protocol", "deploy/live/envoy.yaml", resolved="tls_minimum_protocol_version=TLSv1_0"),
                ee("live-node-disable", "runtime/live.env", resolved="NODE_TLS_REJECT_UNAUTHORIZED=0"),
            ],
        ),
        ScenarioSpec(
            id="N-V7-DOCS-013",
            milestone="M7",
            repo_name="docs-jwt-pem-repo",
            module="both",
            family="negative_docs",
            stack="Docs / tutorials / PKI / JWT",
            domain="Docs repo with masked values, JWT examples, PEM public material, and tutorial snippets.",
            summary="Masked docs values, JWT examples, and PEM public material should remain clean.",
            line_target="5000-7000",
            scan_plans=_both_head(),
            build_plan=_build_plan(NEGATIVE_PY_BUILD),
            expected_absent=[
                ea("masked-docs", "docs/tutorials"),
                ea("jwt-examples", "docs/jwt"),
                ea("public-pem", "docs/pem"),
            ],
        ),
        ScenarioSpec(
            id="N-V7-VENDOR-014",
            milestone="M7",
            repo_name="vendor-sourcemap-noise-repo",
            module="both",
            family="negative_vendor",
            ...
            summary="Vendor, source-map, lockfile, and generated client noise should remain clean.",
            line_target="5000-7000",
            scan_plans=_both_head(),
            build_plan=_build_plan(NEGATIVE_PY_BUILD),
            expected_absent=[
                ea("vendor-bundle", "vendor/"),
                ea("generated-client", "generated/"),
                ea("source-map", "dist/app.min.js.map"),
            ],
        ),
        ScenarioSpec(
            id="N-V7-TESTS-015",
            milestone="M7",
            repo_name="fixtures-mocks-noise-repo",
            module="both",
            family="negative_tests",
            ...
            summary="Nested fixtures, mocks, and fake cert material should remain clean.",
            line_target="5000-7000",
            scan_plans=_both_head(),
            build_plan=_build_plan(NEGATIVE_PY_BUILD),
            expected_absent=[
                ea("fixtures", "fixtures/"),
                ea("mocks", "mocks/"),
                ea("test-certs", "tests/fixtures"),
            ],
        ),
        ScenarioSpec(
            id="N-V7-SAFE-016",
            milestone="M7",
            repo_name="safe-vault-wrapper-repo",
            module="both",
            family="negative_safe",
            stack="Safe wrappers / vault refs / scary naming",
            domain="Safe helper repo with vault references, scary naming, and no live secret or TLS misuse.",
            summary="Vault references and scary naming should remain clean when runtime behavior is safe.",
            line_target="5000-7000",
            scan_plans=_both_head(),
            build_plan=_build_plan(NEGATIVE_PY_BUILD),
            expected_absent=[
                ea("vault-reference", "vault_reference.py"),
                ea("safe-wrapper", "safe_trustall_wrapper.py"),
                ea("safe-docs", "docs/runtime-safety.md"),
            ],
        ),
        ScenarioSpec(
            id="M-V8-009",
            milestone="M8",
            repo_name="payments-mesh-monorepo",
            module="both",
            family="mixed_monorepo",
            ...
            summary="Mixed payments monorepo stressing live Java weak crypto against vendor, openapi, and fixture-heavy surfaces.",
            line_target="10000-20000",
            scan_plans=_both_head(),
            build_plan=_build_plan(MIXED_MONOREPO_BUILD),
            expected_findings=[
        ...
        ScenarioSpec(
            id="M-V8-010",
            milestone="M8",
            repo_name="control-repo-discovery",
            module="both",
            family="mixed_infra_repo",
            stack="App + infra + helm + kustomize + terraform + CI",
            domain="Infra-heavy control repo with branch-only secret, live TLS config, dormant overlays, and generated manifests.",
            summary="Mixed infra control repo stressing branch-only Guardian exposure and live Quantum TLS config amid dormant overlays.",
            line_target="9000-16000",
            scan_plans=_both_head(include_history=True, include_all_branches=True),
            build_plan=_build_plan(INFRA_MIXED_BUILD),
            expected_findings=[
            ...
            expected_absent=[
                ea("generated-manifests", "generated/manifests"),
                ea("example-overlays", "deploy/examples"),
                ea("fixture-overlays", "kustomize/fixtures"),
            ],
            explainability_expectations=[
                ee("quantum-config-trust", "deploy/envoy.yaml", resolved="ACCEPT_UNTRUSTED"),
                ee("quantum-config-protocol", "deploy/envoy.yaml", resolved="tls_minimum_protocol_version=TLSv1_0"),
        ...
        ScenarioSpec(
            id="M-V8-011",
            milestone="M8",
            repo_name="workspace-shared-libs-mixed",
            module="both",
            family="mixed_monorepo",
            ...
            summary="Mixed workspace repo stressing head/history truth, shared-lib helpers, and protected-negative docs surfaces.",
            line_target="10000-20000",
            scan_plans=_both_head(include_history=True, include_all_branches=True),
            build_plan=_build_plan(MIXED_MONOREPO_BUILD),
            expected_findings=[
            ...
            expected_absent=[
                ea("safe-docs", "docs/workspace-history.md"),
                ea("generated-openapi", "generated/openapi"),
                ea("fixtures", "packages/shared-fixtures"),
            ],
            explainability_expectations=[
        ...
        ScenarioSpec(
            id="M-V8-012",
            milestone="M8",
            repo_name="verify-mesh-monorepo",
            module="both",
            family="mixed_monorepo",
            ...
            summary="Mixed monorepo stressing live Python TLS misuse and Java weak crypto beside safe TS wrappers and generated noise.",
            line_target="10000-20000",
            scan_plans=_both_head(),
            build_plan=_build_plan(MIXED_MONOREPO_BUILD),
            expected_findings=[
            ...
            expected_absent=[
                ea("safe-ts-wrapper", "secureTlsWrapper.ts"),
                ea("vendor", "vendor/"),
                ea("generated-openapi", "generated/openapi"),
            ],
            explainability_expectations=[
    ...
    scenario_set = os.environ.get("ARQ_SCENARIO_SET", "all").strip().lower()
    if scenario_set != "frozen85":
